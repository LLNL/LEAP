...
#include "projectors_Joseph_cpu_hip.h"
...
#include "scatter_models_hip.cuh"
#include "geometric_calibration.cuh"
#include "analytic_ray_tracing_gpu_hip.cuh"
#endif
	...
	for (int ichunk = 0; ichunk < numChunks; ichunk++)
	{
		...
		if (numSlices > 0)
		{
			...
			else
			{
				...
				hipSetDevice(params.whichGPUs[omp_get_thread_num()]);
				if ((cudaStatus = hipMalloc((void**)&dev_g, uint64(params.numAngles) * uint64(numRows) * uint64(params.numCols) * sizeof(float))) != hipSuccess)
				{
...
bool tomographicModels::backproject_FBP_multiGPU(float* g, float* f, bool doFBP)
{
	...
	for (int ichunk = 0; ichunk < numChunks; ichunk++)
	{
		...
		else
		{
			...
			hipError_t cudaStatus;
			hipSetDevice(chunk_params.whichGPU);
			float* dev_f = 0;
			if ((cudaStatus = hipMalloc((void**)&dev_f, uint64(chunk_params.numX) * uint64(chunk_params.numY) * uint64(chunk_params.numZ) * sizeof(float))) != hipSuccess)
			{
...
bool tomographicModels::sensitivity(float* f, bool data_on_cpu)
{
	...
	if (params.muSpecified() == true || params.isSymmetric() == true || (params.geometry == parameters::MODULAR && usingSFprojectorsForModularBeam(&params) == false))
	{
		...
		{
			float* dev_g = 0;
			if (hipMalloc((void**)&dev_g, uint64(params.numAngles) * uint64(params.numRows) * uint64(params.numCols) * sizeof(float)) != hipSuccess)
			{
				fprintf(stderr, "hipMalloc(projection[%d,%d,%d]) failed!\n", params.numAngles, params.numRows, params.numCols);
				return false;
			}
