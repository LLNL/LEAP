...
bool projectors::project(float* g, float* f, parameters* params, bool data_on_cpu)
{
	...
	return project(g, f, params, data_on_cpu, volume_on_cpu);
}
...
bool projectors::project(float* g, float* f, parameters* params, bool data_on_cpu, bool volume_on_cpu, bool accumulate)
{
	...
	else if (params->whichGPU >= 0)
	{
		...
		else if (params->geometry == parameters::CONE && params->useSF() == false)
			return project_Joseph_cone(g, f, params, data_on_cpu, volume_on_cpu, accumulate);
		else
...
bool projectors::backproject(float* g, float* f, parameters* params, bool data_on_cpu)
{
	...
	bool has_unified_memory = physically_shared_memory(params->whichGPU);
	bool volume_on_cpu = data_on_cpu && !has_unified_memory;
	return backproject(g, f, params, data_on_cpu, volume_on_cpu);
}
...
bool projectors::backproject(float* g, float* f, parameters* params, bool data_on_cpu, bool volume_on_cpu, bool accumulate)
{
	...
	else if (params->whichGPU >= 0)
	{
		...
		if (params->doDBP)
			params->offsetX += 0.5*params->voxelWidth;
		bool retVal = true;
		if (params->isSymmetric())
		{
			if (params->doWeightedBackprojection)
				...
				retVal = backproject_symmetric(g, f, params, data_on_cpu);
		}
		else if (params->muSpecified())
			retVal = backproject_attenuated(g, f, params, data_on_cpu, volume_on_cpu);
		else if (params->whichProjector == parameters::VOXEL_DRIVEN /* && (params->helicalPitch == 0.0 || params->doWeightedBackprojection == false)*/)
			retVal = backproject_VD(g, f, params, data_on_cpu, volume_on_cpu, accumulate);
		else if (params->geometry == parameters::MODULAR)
			retVal = backproject_Joseph_modular(g, f, params, data_on_cpu, volume_on_cpu, accumulate);
		else
		{
			if (params->useSF())
				retVal = backproject_SF(g, f, params, data_on_cpu, volume_on_cpu, accumulate);
			else
				retVal = backproject_VD(g, f, params, data_on_cpu, volume_on_cpu, accumulate);
		}
		if (params->doDBP)
...
bool projectors::weightedBackproject(float* g, float* f, parameters* params, bool data_on_cpu)
{
	...
	bool has_unified_memory = physically_shared_memory(params->whichGPU);
	bool volume_on_cpu = data_on_cpu && !has_unified_memory;
	return weightedBackproject(g, f, params, data_on_cpu, volume_on_cpu);
}
