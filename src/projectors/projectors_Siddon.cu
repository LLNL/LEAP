...
__device__ float projectLine(TEX_DATA f, int4 N_f, float4 T_f, float4 startVals_f, float4 pos, float4 traj)
{
	float val = 0.0;
		...
		if (y_inc > 0.0f)
		{
			if (z_inc > 0.0f)
			{
				for (int ix = 0; ix < N_f.x; ix++)
				{
					...
					val += max(0.0f, min(1.0f, min((iy + 0.5f - y_low) / y_inc, (iz + 0.5f - z_low) / z_inc)) - max(0.0f, max((iy - 0.5f - y_low) / y_inc, (iz - 0.5f - z_low) / z_inc)))*TEX3D(f, iz, iy, ix)
						+ max(0.0f, min(1.0f, min((iy + 1 + 0.5f - y_low) / y_inc, (iz + 0.5f - z_low) / z_inc)) - max(0.0f, max((iy + 1 - 0.5f - y_low) / y_inc, (iz - 0.5f - z_low) / z_inc)))*TEX3D(f, iz, iy + 1, ix)
						+ max(0.0f, min(1.0f, min((iy + 0.5f - y_low) / y_inc, (iz + 1 + 0.5f - z_low) / z_inc)) - max(0.0f, max((iy - 0.5f - y_low) / y_inc, (iz + 1 - 0.5f - z_low) / z_inc)))*TEX3D(f, iz + 1, iy, ix)
						+ max(0.0f, min(1.0f, min((iy + 1 + 0.5f - y_low) / y_inc, (iz + 1 + 0.5f - z_low) / z_inc)) - max(0.0f, max((iy + 1 - 0.5f - y_low) / y_inc, (iz + 1 - 0.5f - z_low) / z_inc)))*TEX3D(f, iz + 1, iy + 1, ix);
			...
			else //if (z_inc == 0.0f)
			{
				for (int ix = 0; ix < N_f.x; ix++)
				{
					...
					val += max(0.0f, min(1.0f, (iy + 0.5f - y_low) / y_inc) - max(0.0f, (iy - 0.5f - y_low) / y_inc))*TEX3D(f, iz, iy, ix)
						+ max(0.0f, min(1.0f, (iy + 1 + 0.5f - y_low) / y_inc) - max(0.0f, (iy + 1 - 0.5f - y_low) / y_inc))*TEX3D(f, iz, iy + 1, ix);
		...
		else // y_inc == 0.0f
		{
			if (z_inc > 0.0f)
			{
				for (int ix = 0; ix < N_f.x; ix++)
				{
					...
					val += max(0.0f, min(1.0f, (iz + 0.5f - z_low) / z_inc) - max(0.0f, (iz - 0.5f - z_low) / z_inc))*TEX3D(f, iz, iy, ix)
						+ max(0.0f, min(1.0f, (iz + 1 + 0.5f - z_low) / z_inc) - max(0.0f, (iz + 1 - 0.5f - z_low) / z_inc))*TEX3D(f, iz + 1, iy, ix);
		...
		if (x_inc > 0.0f)
		{
			if (z_inc > 0.0f)
			{
				for (int iy = 0; iy < N_f.y; iy++)
				{
					...
					val += max(0.0f, min(1.0f, min((ix + 0.5f - x_low) / x_inc, (iz + 0.5f - z_low) / z_inc)) - max(0.0f, max((ix - 0.5f - x_low) / x_inc, (iz - 0.5f - z_low) / z_inc)))*TEX3D(f, iz, iy, ix)
						+ max(0.0f, min(1.0f, min((ix + 1 + 0.5f - x_low) / x_inc, (iz + 0.5f - z_low) / z_inc)) - max(0.0f, max((ix + 1 - 0.5f - x_low) / x_inc, (iz - 0.5f - z_low) / z_inc)))*TEX3D(f, iz, iy, ix + 1)
						+ max(0.0f, min(1.0f, min((ix + 0.5f - x_low) / x_inc, (iz + 1 + 0.5f - z_low) / z_inc)) - max(0.0f, max((ix - 0.5f - x_low) / x_inc, (iz + 1 - 0.5f - z_low) / z_inc)))*TEX3D(f, iz + 1, iy, ix)
						+ max(0.0f, min(1.0f, min((ix + 1 + 0.5f - x_low) / x_inc, (iz + 1 + 0.5f - z_low) / z_inc)) - max(0.0f, max((ix + 1 - 0.5f - x_low) / x_inc, (iz + 1 - 0.5f - z_low) / z_inc)))*TEX3D(f, iz + 1, iy, ix + 1);
			...
			else //if (z_inc == 0.0f)
			{
				for (int iy = 0; iy < N_f.y; iy++)
				{
					...
					val += max(0.0f, min(1.0f, (ix + 0.5f - x_low) / x_inc) - max(0.0f, (ix - 0.5f - x_low) / x_inc))*TEX3D(f, iz, iy, ix)
						+ max(0.0f, min(1.0f, (ix + 1 + 0.5f - x_low) / x_inc) - max(0.0f, (ix + 1 - 0.5f - x_low) / x_inc))*TEX3D(f, iz, iy, ix + 1);
		...
		if (x_inc > 0.0f)
		{
			if (y_inc > 0.0f)
			{
				for (int iz = 0; iz < N_f.z; iz++)
				{
					...
					val += max(0.0f, min(1.0f, min((ix + 0.5f - x_low) / x_inc, (iy + 0.5f - y_low) / y_inc)) - max(0.0f, max((ix - 0.5f - x_low) / x_inc, (iy - 0.5f - y_low) / y_inc)))*TEX3D(f, iz, iy, ix)
						+ max(0.0f, min(1.0f, min((ix + 1 + 0.5f - x_low) / x_inc, (iy + 0.5f - y_low) / y_inc)) - max(0.0f, max((ix + 1 - 0.5f - x_low) / x_inc, (iy - 0.5f - y_low) / y_inc)))*TEX3D(f, iz, iy, ix + 1)
						+ max(0.0f, min(1.0f, min((ix + 0.5f - x_low) / x_inc, (iy + 1 + 0.5f - y_low) / y_inc)) - max(0.0f, max((ix - 0.5f - x_low) / x_inc, (iy + 1 - 0.5f - y_low) / y_inc)))*TEX3D(f, iz, iy + 1, ix)
						+ max(0.0f, min(1.0f, min((ix + 1 + 0.5f - x_low) / x_inc, (iy + 1 + 0.5f - y_low) / y_inc)) - max(0.0f, max((ix + 1 - 0.5f - x_low) / x_inc, (iy + 1 - 0.5f - y_low) / y_inc)))*TEX3D(f, iz, iy + 1, ix + 1);
...
__device__ float projectLine_ZYX(TEX_DATA f, int4 N_f, float4 T_f, float4 startVals_f, float4 pos, float4 traj)
{
	float val = 0.0;
	if (fabs(traj.x) >= fabs(traj.y) && fabs(traj.x) >= fabs(traj.z))
	{
		...
		if (y_inc > 0.0f)
		{
			if (z_inc > 0.0f)
			{
				for (int ix = 0; ix < N_f.x; ix++)
				{
					...
					val += max(0.0f, min(1.0f, min((iy + 0.5f - y_low) / y_inc, (iz + 0.5f - z_low) / z_inc)) - max(0.0f, max((iy - 0.5f - y_low) / y_inc, (iz - 0.5f - z_low) / z_inc))) * TEX3D(f, ix, iy, iz)
						+ max(0.0f, min(1.0f, min((iy + 1 + 0.5f - y_low) / y_inc, (iz + 0.5f - z_low) / z_inc)) - max(0.0f, max((iy + 1 - 0.5f - y_low) / y_inc, (iz - 0.5f - z_low) / z_inc))) * TEX3D(f, ix, iy + 1, iz)
						+ max(0.0f, min(1.0f, min((iy + 0.5f - y_low) / y_inc, (iz + 1 + 0.5f - z_low) / z_inc)) - max(0.0f, max((iy - 0.5f - y_low) / y_inc, (iz + 1 - 0.5f - z_low) / z_inc))) * TEX3D(f, ix, iy, iz + 1)
						+ max(0.0f, min(1.0f, min((iy + 1 + 0.5f - y_low) / y_inc, (iz + 1 + 0.5f - z_low) / z_inc)) - max(0.0f, max((iy + 1 - 0.5f - y_low) / y_inc, (iz + 1 - 0.5f - z_low) / z_inc))) * TEX3D(f, ix, iy + 1, iz + 1);
			...
			else //if (z_inc == 0.0f)
			{
				for (int ix = 0; ix < N_f.x; ix++)
				{
					...
					val += max(0.0f, min(1.0f, (iy + 0.5f - y_low) / y_inc) - max(0.0f, (iy - 0.5f - y_low) / y_inc)) * TEX3D(f, ix, iy, iz)
						+ max(0.0f, min(1.0f, (iy + 1 + 0.5f - y_low) / y_inc) - max(0.0f, (iy + 1 - 0.5f - y_low) / y_inc)) * TEX3D(f, ix, iy + 1, iz);
		...
		else // y_inc == 0.0f
		{
			if (z_inc > 0.0f)
			{
				for (int ix = 0; ix < N_f.x; ix++)
				{
					...
					val += max(0.0f, min(1.0f, (iz + 0.5f - z_low) / z_inc) - max(0.0f, (iz - 0.5f - z_low) / z_inc)) * TEX3D(f, ix, iy, iz)
						+ max(0.0f, min(1.0f, (iz + 1 + 0.5f - z_low) / z_inc) - max(0.0f, (iz + 1 - 0.5f - z_low) / z_inc)) * TEX3D(f, ix, iy, iz + 1);
		...
		if (x_inc > 0.0f)
		{
			if (z_inc > 0.0f)
			{
				for (int iy = 0; iy < N_f.y; iy++)
				{
					...
					val += max(0.0f, min(1.0f, min((ix + 0.5f - x_low) / x_inc, (iz + 0.5f - z_low) / z_inc)) - max(0.0f, max((ix - 0.5f - x_low) / x_inc, (iz - 0.5f - z_low) / z_inc))) * TEX3D(f, ix, iy, iz)
						+ max(0.0f, min(1.0f, min((ix + 1 + 0.5f - x_low) / x_inc, (iz + 0.5f - z_low) / z_inc)) - max(0.0f, max((ix + 1 - 0.5f - x_low) / x_inc, (iz - 0.5f - z_low) / z_inc))) * TEX3D(f, ix + 1, iy, iz)
						+ max(0.0f, min(1.0f, min((ix + 0.5f - x_low) / x_inc, (iz + 1 + 0.5f - z_low) / z_inc)) - max(0.0f, max((ix - 0.5f - x_low) / x_inc, (iz + 1 - 0.5f - z_low) / z_inc))) * TEX3D(f, ix, iy, iz + 1)
						+ max(0.0f, min(1.0f, min((ix + 1 + 0.5f - x_low) / x_inc, (iz + 1 + 0.5f - z_low) / z_inc)) - max(0.0f, max((ix + 1 - 0.5f - x_low) / x_inc, (iz + 1 - 0.5f - z_low) / z_inc))) * TEX3D(f, ix + 1, iy, iz + 1);
			...
			else //if (z_inc == 0.0f)
			{
				for (int iy = 0; iy < N_f.y; iy++)
				{
					...
					val += max(0.0f, min(1.0f, (ix + 0.5f - x_low) / x_inc) - max(0.0f, (ix - 0.5f - x_low) / x_inc)) * TEX3D(f, ix, iy, iz)
						+ max(0.0f, min(1.0f, (ix + 1 + 0.5f - x_low) / x_inc) - max(0.0f, (ix + 1 - 0.5f - x_low) / x_inc)) * TEX3D(f, ix + 1, iy, iz);
		...
		if (x_inc > 0.0f)
		{
			if (y_inc > 0.0f)
			{
				for (int iz = 0; iz < N_f.z; iz++)
				{
					...
					val += max(0.0f, min(1.0f, min((ix + 0.5f - x_low) / x_inc, (iy + 0.5f - y_low) / y_inc)) - max(0.0f, max((ix - 0.5f - x_low) / x_inc, (iy - 0.5f - y_low) / y_inc))) * TEX3D(f, ix, iy, iz)
						+ max(0.0f, min(1.0f, min((ix + 1 + 0.5f - x_low) / x_inc, (iy + 0.5f - y_low) / y_inc)) - max(0.0f, max((ix + 1 - 0.5f - x_low) / x_inc, (iy - 0.5f - y_low) / y_inc))) * TEX3D(f, ix + 1, iy, iz)
						+ max(0.0f, min(1.0f, min((ix + 0.5f - x_low) / x_inc, (iy + 1 + 0.5f - y_low) / y_inc)) - max(0.0f, max((ix - 0.5f - x_low) / x_inc, (iy + 1 - 0.5f - y_low) / y_inc))) * TEX3D(f, ix, iy + 1, iz)
						+ max(0.0f, min(1.0f, min((ix + 1 + 0.5f - x_low) / x_inc, (iy + 1 + 0.5f - y_low) / y_inc)) - max(0.0f, max((ix + 1 - 0.5f - x_low) / x_inc, (iy + 1 - 0.5f - y_low) / y_inc))) * TEX3D(f, ix + 1, iy + 1, iz);
...
__global__ void fanBeamBackprojectorKernel(TEX_DATA g, int4 N_g, float4 T_g, float4 startVals_g, float* f, int4 N_f, float4 T_f, float4 startVals_f, float R, float D, float rFOVsq, float* phis, int volumeDimensionOrder)
{
	...
	for (int iphi = 0; iphi < N_g.x; iphi++)
	{
		...
		for (int iu = u_arg_mid - searchWidth_u; iu <= u_arg_mid + searchWidth_u; iu++)
		{
			...
			val += max(0.0f, t_max - t_min) * TEX3D(g, iu, iv, iphi);
		}
	}
	f[ind] = val;
}

__global__ void fanBeamProjectorKernel(float* g, int4 N_g, float4 T_g, float4 startVals_g, TEX_DATA f, int4 N_f, float4 T_f, float4 startVals_f, float R, float D, float* phis, int volumeDimensionOrder)
{
	const int i = threadIdx.x + blockIdx.x * blockDim.x;
	const int j = threadIdx.y + blockIdx.y * blockDim.y;
	const int k = threadIdx.z + blockIdx.z * blockDim.z;
...
__global__ void coneBeamBackprojectorKernel(TEX_DATA g, int4 N_g, float4 T_g, float4 startVals_g, float* f, int4 N_f, float4 T_f, float4 startVals_f, float R, float D, float rFOVsq, float* phis, int volumeDimensionOrder)
{
	const int i = threadIdx.x + blockIdx.x * blockDim.x;
	const int j = threadIdx.y + blockIdx.y * blockDim.y;
	const int k = threadIdx.z + blockIdx.z * blockDim.z;
...
__global__ void coneBeamProjectorKernel(float* g, int4 N_g, float4 T_g, float4 startVals_g, TEX_DATA f, int4 N_f, float4 T_f, float4 startVals_f, float R, float D, float* phis, int volumeDimensionOrder)
{
	const int i = threadIdx.x + blockIdx.x * blockDim.x;
	const int j = threadIdx.y + blockIdx.y * blockDim.y;
	const int k = threadIdx.z + blockIdx.z * blockDim.z;
...
__global__ void parallelBeamBackprojectorKernel(TEX_DATA g, int4 N_g, float4 T_g, float4 startVals_g, float* f, int4 N_f, float4 T_f, float4 startVals_f, float rFOVsq, float* phis, int volumeDimensionOrder)
{
	const int i = threadIdx.x + blockIdx.x * blockDim.x;
	const int j = threadIdx.y + blockIdx.y * blockDim.y;
	const int k = threadIdx.z + blockIdx.z * blockDim.z;
...
__global__ void parallelBeamProjectorKernel(float* g, int4 N_g, float4 T_g, float4 startVals_g, TEX_DATA f, int4 N_f, float4 T_f, float4 startVals_f, float* phis, int volumeDimensionOrder)
{
	const int i = threadIdx.x + blockIdx.x * blockDim.x;
	const int j = threadIdx.y + blockIdx.y * blockDim.y;
	const int k = threadIdx.z + blockIdx.z * blockDim.z;
...
__global__ void modularBeamBackprojectorKernel(TEX_DATA g, int4 N_g, float4 T_g, float4 startVals_g, float* f, int4 N_f, float4 T_f, float4 startVals_f, float* sourcePositions, float* moduleCenters, float* rowVectors, float* colVectors, int volumeDimensionOrder)
{
	const int i = threadIdx.x + blockIdx.x * blockDim.x;
	const int j = threadIdx.y + blockIdx.y * blockDim.y;
	const int k = threadIdx.z + blockIdx.z * blockDim.z;
...
__global__ void modularBeamProjectorKernel(float* g, int4 N_g, float4 T_g, float4 startVals_g, TEX_DATA f, int4 N_f, float4 T_f, float4 startVals_f, float* sourcePositions, float* moduleCenters, float* rowVectors, float* colVectors, int volumeDimensionOrder)
{
	const int i = threadIdx.x + blockIdx.x * blockDim.x;
	const int j = threadIdx.y + blockIdx.y * blockDim.y;
	const int k = threadIdx.z + blockIdx.z * blockDim.z;
...
bool backproject_modular(float* g, float *&f, parameters* params, bool data_on_cpu)
{
	...
	TEX_DATA d_data_txt = {};
	TEX_ARRAY d_data_array = loadTexture(d_data_txt, dev_g, N_g, false, false);
