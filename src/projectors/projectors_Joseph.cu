...
//*
__global__ void modularBeamProjectorKernel_SF(float* g, int4 N_g, float4 T_g, float4 startVals_g, TEX_DATA f, int4 N_f, float4 T_f, float4 startVals_f, float* sourcePositions, float* moduleCenters, float* rowVectors, float* colVectors, int volumeDimensionOrder, const float rFOVsq, const bool accum, int g_rowStride)
{
    ...
    if (fabs(r.y) > fabs(r.x))
    {
        ...
        if (accum)
            g[uint64(l) * uint64(N_g.z) * uint64(g_rowStride) + uint64(m * N_g.z + n)] += T_f.x * sqrtf((r.y*r.y + r.x*r.x)*(r.y*r.y + r.z*r.z)) * fabs(r_y_inv*r_y_inv) * g_output;
        else
            g[uint64(l) * uint64(N_g.z) * uint64(g_rowStride) + uint64(m * N_g.z + n)] = T_f.x * sqrtf((r.y*r.y + r.x*r.x)*(r.y*r.y + r.z*r.z)) * fabs(r_y_inv*r_y_inv) * g_output;
    }
    else
    {
        ...
        if (accum)
            g[uint64(l) * uint64(N_g.z) * uint64(g_rowStride) + uint64(m * N_g.z + n)] += T_f.x * sqrtf((r.x*r.x + r.y*r.y)*(r.x*r.x + r.z*r.z)) * fabs(r_x_inv*r_x_inv) * g_output;
        else
            g[uint64(l) * uint64(N_g.z) * uint64(g_rowStride) + uint64(m * N_g.z + n)] = T_f.x * sqrtf((r.x*r.x + r.y*r.y)*(r.x*r.x + r.z*r.z)) * fabs(r_x_inv*r_x_inv) * g_output;
    }
}
...
/*
__global__ void modularBeamProjectorKernel_SF(float* g, int4 N_g, float4 T_g, float4 startVals_g, TEX_DATA f, int4 N_f, float4 T_f, float4 startVals_f, float* sourcePositions, float* moduleCenters, float* rowVectors, float* colVectors, int volumeDimensionOrder, const float rFOVsq)
{
    const int l = threadIdx.x + blockIdx.x * blockDim.x;
    const int m = threadIdx.y + blockIdx.y * blockDim.y;
    const int n = threadIdx.z + blockIdx.z * blockDim.z;
    ...
    if (fabs(u * cos_phi - sin_phi) > fabs(u * sin_phi + cos_phi))
    {
        ...
        g[uint64(l) * uint64(N_g.z) * uint64(g_rowStride) + uint64(m * N_g.z + n)] = T_f.x * sqrt(1.0f + u * u) / fabs(u * cos_phi - sin_phi) * g_output;
    }
    else
    {
        ...
        g[uint64(l) * uint64(N_g.z) * uint64(g_rowStride) + uint64(m * N_g.z + n)] = T_f.x * sqrt(1.0f + u * u) / fabs(u * sin_phi + cos_phi) * g_output;
    }
}
//*/

__global__ void modularBeamBackprojectorKernel_SF_stack(TEX_DATA g, int4 N_g, float4 T_g, float4 startVals_g, float* f, int4 N_f, float4 T_f, float4 startVals_f, float* sourcePositions, float* moduleCenters, float* rowVectors, float* colVectors, int volumeDimensionOrder, const float rFOV_sq, const bool accum)
{
    ...
    for (int iphi = 0; iphi < N_g.x; iphi++)
    {
        ...
        if (fabs(x - sourcePosition.x) > fabs(y - sourcePosition.y))
        {
            for (int k_offset = 0; k_offset < numZ; k_offset++)
            {
                ...
                if (z_high_A > 1.0f)
                {
                    vals[k_offset] += (TEX3D(g, u_ind_first, row_high_A + v_oneAndTwo, L) * horizontalWeights_0_A
                        ...
                        + (TEX3D(g, u_ind_first, row_high_plus_two_A, L) * horizontalWeights_0_A
                            + TEX3D(g, u_ind_last, row_high_plus_two_A, L) * horizontalWeights_1_A) * (z_high_A - 1.0f);
                }
                else
                {
                    vals[k_offset] += (TEX3D(g, u_ind_first, row_high_A + v_oneAndTwo, L) * horizontalWeights_0_A
...
//*
__global__ void modularBeamBackprojectorKernel_SF(TEX_DATA g, int4 N_g, float4 T_g, float4 startVals_g, float* f, int4 N_f, float4 T_f, float4 startVals_f, float* sourcePositions, float* moduleCenters, float* rowVectors, float* colVectors, int volumeDimensionOrder, const float rFOV_sq, const bool accum)
{
    const int i = threadIdx.x + blockIdx.x * blockDim.x;
    const int j = threadIdx.y + blockIdx.y * blockDim.y;
...
/*
__global__ void modularBeamBackprojectorKernel_SF(TEX_DATA g, int4 N_g, float4 T_g, float4 startVals_g, float* f, int4 N_f, float4 T_f, float4 startVals_f, float* sourcePositions, float* moduleCenters, float* rowVectors, float* colVectors, int volumeDimensionOrder, const float rFOV_sq)
{
    const int i = threadIdx.x + blockIdx.x * blockDim.x;
    const int j = threadIdx.y + blockIdx.y * blockDim.y;
    const int k = threadIdx.z + blockIdx.z * blockDim.z;
...
__global__ void modularBeamBackprojectorKernel_eSF_stack(TEX_DATA g, int4 N_g, float4 T_g, float4 startVals_g, float* f, int4 N_f, float4 T_f, float4 startVals_f, float* sourcePositions, float* moduleCenters, float* rowVectors, float* colVectors, int volumeDimensionOrder, const float rFOV_sq, const bool accum)
{
    const int i = threadIdx.x + blockIdx.x * blockDim.x;
    const int j = threadIdx.y + blockIdx.y * blockDim.y;
    const int k = (threadIdx.z + blockIdx.z * blockDim.z) * NUM_SLICES_PER_THREAD;
...
__global__ void modularBeamBackprojectorKernel_eSF(TEX_DATA g, int4 N_g, float4 T_g, float4 startVals_g, float* f, int4 N_f, float4 T_f, float4 startVals_f, float* sourcePositions, float* moduleCenters, float* rowVectors, float* colVectors, int volumeDimensionOrder, const float rFOV_sq, const bool accum)
{
    const int i = threadIdx.x + blockIdx.x * blockDim.x;
    const int j = threadIdx.y + blockIdx.y * blockDim.y;
    const int k = threadIdx.z + blockIdx.z * blockDim.z;
            ...
            for (int iu = iu_min; iu <= iu_max; iu += 2)
            {
                ...
                if (uWeight + uWeight_2 > 0.0f)
                {
                    ...
                    for (int iv = iv_min; iv <= iv_max; iv += 2)
                    {
                        ...
                        {
                            const float vshift_12 = vWeight_2 / (vWeight + vWeight_2);
                            val += TEX3D(g, iu + ushift_12 + 0.5f, iv + vshift_12 + 0.5f, iphi + 0.5f) * (uWeight + uWeight_2) * (vWeight + vWeight_2);
                        }
                    }
                }
            }
...
__device__ float lineIntegral_Joseph_ZYX(TEX_DATA mu, const int4 N, const float4 T, const float4 startVal, const float3 p, const float3 dst)
{
...
__device__ float lineIntegral_Joseph_XYZ(TEX_DATA mu, const int4 N, const float4 T, const float4 startVal, const float3 p, const float3 dst)
{
    // NOTE: assumes that T.x == T.y == T.z
    const float3 ip = make_float3((p.x - startVal.x) / T.x, (p.y - startVal.y) / T.y,
        (p.z - startVal.z) / T.z);
    ...
    if (fabsf(r.x) >= fabsf(r.y) && fabsf(r.x) >= fabsf(r.z))
    {
        ...
        if (r.x > 0.0f)
        {
            ...
            val = TEX3D(mu, iz_start + 0.5f, iy_start + 0.5f, float(ix_start) + 0.5f) *
                ((float(ix_start) - 0.5f) - max(-0.5f, ip.x));
            ...
            for (int ix = ix_start; ix <= ix_max; ix++)
                val += TEX3D(mu, iz_offset + ir.z * float(ix), iy_offset + ir.y * float(ix), float(ix) + 0.5f);
        }
        else
        {
            if (ip.x <= -0.5f) return 0.0f;
    ...
    else if (fabsf(r.y) >= fabsf(r.z))
    {
        ...
        if (r.y > 0.0f)
        {
            ...
            val = TEX3D(mu, iz_start + 0.5f, float(iy_start) + 0.5f, ix_start + 0.5f) *
                ((float(iy_start) - 0.5f) - max(-0.5f, ip.y));

            const float ix_offset = ix_start - ir.x * float(iy_start) + 0.5f;
            const float iz_offset = iz_start - ir.z * float(iy_start) + 0.5f;
            for (int iy = iy_start; iy <= iy_max; iy++)
                val += TEX3D(mu, iz_offset + ir.z * float(iy), float(iy) + 0.5f, ix_offset + ir.x * float(iy));
        }
        else
        {
            if (ip.y <= -0.5f) return 0.0f;
            ...
            val = TEX3D(mu, iz_start + 0.5f, iy_start + 0.5f, ix_start + 0.5f) *
                (min((float(N.y) - 0.5f), ip.y) - (float(iy_start) + 0.5f));
    ...
    else
    {
        ...
        if (r.z > 0.0f)
        {
            ...
            const float iy_offset = iy_start - ir.y * float(iz_start) + 0.5f;
            for (int iz = iz_start; iz <= iz_max; iz++)
                val += TEX3D(mu, float(iz) + 0.5f, iy_offset + ir.y * float(iz), ix_offset + ir.x * float(iz));
        }
        else
        {
            if (ip.z <= -0.5f) return 0.0f;
            ...
            val = TEX3D(mu, float(iz_start) + 0.5f, iy_start + 0.5f, ix_start + 0.5f) *
                (min((float(N.z) - 0.5f), ip.z) - (float(iz_start) + 0.5f));
...
__global__ void modularBeamParallelJosephBackprojectorKernel(TEX_DATA g, int4 N_g, float4 T_g, float4 startVals_g, float* f, int4 N_f, float4 T_f, float4 startVals_f, float* sourcePositions, float* moduleCenters, float* rowVectors, float* colVectors, int volumeDimensionOrder, const float rFOV_sq, const bool accum)
{
    const int i = threadIdx.x + blockIdx.x * blockDim.x;
    const int j = threadIdx.y + blockIdx.y * blockDim.y;
    const int k = threadIdx.z + blockIdx.z * blockDim.z;
        ...
        if (fabs(r.x) >= max(fabs(r.y), fabs(r.z)))
        {
            ...
            for (int iv = max(0, v_ind - searchWidth_v); iv <= v_ind + searchWidth_v; iv++)
            {
                ...
                for (int iu = max(0, u_ind - searchWidth_u); iu <= u_ind + searchWidth_u; iu++)
                {
                    ...
                    {
                        sum_weights += dy * dz;
                        val_local += dy * dz * TEX3D(g, iu, iv, iphi);// *rayWeight;
                        //val += dy * dz * TEX3D(g, iu, iv, iphi);
                    }
                }
            }
        }
        else if (fabs(r.y) >= fabs(r.z))
        {
            ...
            for (int iv = max(0, v_ind - searchWidth_v); iv <= v_ind + searchWidth_v; iv++)
            {
                ...
                for (int iu = max(0, u_ind - searchWidth_u); iu <= u_ind + searchWidth_u; iu++)
                {
                    ...
                    {
                        sum_weights += dx * dz;
                        val_local += dx * dz * TEX3D(g, iu, iv, iphi);// *rayWeight;
                        //val += dx * dz * TEX3D(g, iu, iv, iphi);
                    }
                }
            }
        }
        else
        {
            ...
            for (int iv = max(0, v_ind - searchWidth_v); iv <= v_ind + searchWidth_v; iv++)
            {
                ...
                for (int iu = max(0, u_ind - searchWidth_u); iu <= u_ind + searchWidth_u; iu++)
                {
                    ...
                    {
                        sum_weights += dx * dy;
                        val_local += dx * dy * TEX3D(g, iu, iv, iphi);// *rayWeight;
                        //val += dx * dy * TEX3D(g, iu, iv, iphi);
                    }
                }
            }
        }
...
__global__ void modularBeamJosephBackprojectorKernel(TEX_DATA g, int4 N_g, float4 T_g, float4 startVals_g, float* f, int4 N_f, float4 T_f, float4 startVals_f, float* sourcePositions, float* moduleCenters, float* rowVectors, float* colVectors, int volumeDimensionOrder, const float rFOV_sq, const bool accum)
{
    const int i = threadIdx.x + blockIdx.x * blockDim.x;
    const int j = threadIdx.y + blockIdx.y * blockDim.y;
    const int k = threadIdx.z + blockIdx.z * blockDim.z;
    ...
    for (int iphi = 0; iphi < N_g.x; iphi++)
    {
        ...
        if (sum_weights > 0.0f)
            ...
            val += footprintSize_u * footprintSize_v * TEX3D(g, u_ind, v_ind, iphi);
        //*/
        ...
        if (fabs(r.x) >= max(fabs(r.y), fabs(r.z)))
        {
            ...
            for (int iv = max(0, v_ind - searchWidth_v); iv <= v_ind + searchWidth_v; iv++)
            {
                ...
                for (int iu = max(0, u_ind - searchWidth_u); iu <= u_ind + searchWidth_u; iu++)
                {
                    ...
                    if (dy > 0.0f && dz > 0.0f)
                    {
                        //val += sqrtf(trueRay_x * trueRay_x + trueRay_y * trueRay_y + trueRay_z * trueRay_z) * fabs(trueRay_x_inv) * dy * dz * TEX3D(g, iu, iv, iphi);
                        val += rayWeight * dy * dz * TEX3D(g, iu, iv, iphi);
                        //val += dy * dz * TEX3D(g, iu, iv, iphi);
                    }
                }
            }
        ...
        else if (fabs(r.y) >= fabs(r.z))
        {
            ...
            for (int iv = max(0, v_ind - searchWidth_v); iv <= v_ind + searchWidth_v; iv++)
            {
                ...
                for (int iu = max(0, u_ind - searchWidth_u); iu <= u_ind + searchWidth_u; iu++)
                {
                    ...
                    if (dx > 0.0f && dz > 0.0f)
                    {
                        //val += sqrtf(trueRay_y * trueRay_y + trueRay_x * trueRay_x + trueRay_z * trueRay_z) * fabs(trueRay_y_inv) * dx * dz * TEX3D(g, iu, iv, iphi);
                        val += rayWeight * dx * dz * TEX3D(g, iu, iv, iphi);
                        //val += dx * dz * TEX3D(g, iu, iv, iphi);
                    }
                }
            }
        ...
        else
        {
            ...
            for (int iv = max(0, v_ind - searchWidth_v); iv <= v_ind + searchWidth_v; iv++)
            {
                ...
                for (int iu = max(0, u_ind - searchWidth_u); iu <= u_ind + searchWidth_u; iu++)
                {
                    ...
                    if (dx > 0.0f && dy > 0.0f)
                    {
                        //val += sqrtf(trueRay_z * trueRay_z + trueRay_x * trueRay_x + trueRay_y * trueRay_y) * fabs(trueRay_z_inv) * dx * dy * TEX3D(g, iu, iv, iphi);
                        val += rayWeight * dx * dy * TEX3D(g, iu, iv, iphi);
                        //val += dx * dy * TEX3D(g, iu, iv, iphi);
                    }
                }
            }
...
//#####################################################################################################################
__global__ void modularBeamBackprojectorKernel(TEX_DATA g, int4 N_g, float4 T_g, float4 startVals_g, float* f, int4 N_f, float4 T_f, float4 startVals_f, float* sourcePositions, float* moduleCenters, float* rowVectors, float* colVectors, int volumeDimensionOrder, const float rFOV_sq, const bool accum)
{
    const int i = threadIdx.x + blockIdx.x * blockDim.x;
    const int j = threadIdx.y + blockIdx.y * blockDim.y;
    const int k = threadIdx.z + blockIdx.z * blockDim.z;
...
__global__ void modularBeamJosephProjectorKernel(float* g, int4 N_g, float4 T_g, float4 startVals_g, TEX_DATA f, int4 N_f, float4 T_f, float4 startVals_f, float* sourcePositions, float* moduleCenters, float* rowVectors, float* colVectors, int volumeDimensionOrder, const bool accum, int g_rowStride)
{
    const int i = threadIdx.x + blockIdx.x * blockDim.x;
    const int j = threadIdx.y + blockIdx.y * blockDim.y;
    const int k = threadIdx.z + blockIdx.z * blockDim.z;
    ...
    {
        if (volumeDimensionOrder == 0)
            g[uint64(i) * uint64(N_g.z) * uint64(g_rowStride) + uint64(j * N_g.z + k)] += lineIntegral_Joseph_XYZ(f, N_f, T_f, startVals_f, edgePos, dst);
        else
            g[uint64(i) * uint64(N_g.z) * uint64(g_rowStride) + uint64(j * N_g.z + k)] += lineIntegral_Joseph_ZYX(f, N_f, T_f, startVals_f, edgePos, dst);
    }
    else
    {
        if (volumeDimensionOrder == 0)
...
bool project_Joseph_modular(float*& g, float* f, parameters* params, bool data_on_cpu, bool volume_on_cpu, bool accum)
{
    ...
    if (params->modularbeamIsAxiallyAligned() == true && useSF == true)
    {
        ...
        modularBeamProjectorKernel_SF <<< dimGrid, dimBlock >>> (dev_g_out, N_g, T_g, startVal_g, d_data_txt, N_f, T_f, startVal_f, dev_sourcePositions, dev_moduleCenters, dev_rowVectors, dev_colVectors, params->volumeDimensionOrder, rFOV_sq, accum, g_rowStride);
    ...
    {
        modularBeamJosephProjectorKernel <<< dimGrid, dimBlock >>> (dev_g_out, N_g, T_g, startVal_g, d_data_txt, N_f, T_f, startVal_f, dev_sourcePositions, dev_moduleCenters, dev_rowVectors, dev_colVectors, params->volumeDimensionOrder, accum, g_rowStride);
    }
...
__global__ void coneBeamProjectorKernel_Joseph(float* __restrict__ g, const int4 N_g, const float4 T_g, const float4 startVals_g, TEX_DATA f, const int4 N_f, const float4 T_f, const float4 startVals_f, const float R, const float D, const float tau, const float rFOVsq, const float* __restrict__ phis, const int volumeDimensionOrder, bool accum, int g_rowStride)
{
    const int l = threadIdx.x + blockIdx.x * blockDim.x;
    const int m = threadIdx.y + blockIdx.y * blockDim.y;
    const int n = threadIdx.z + blockIdx.z * blockDim.z;
    ...
    const float3 sourcePos = make_float3(R*cos_phi + tau*sin_phi, R*sin_phi - tau*cos_phi, phi * T_g.w + startVals_g.w);

    const float3 v_vec = make_float3(cos_phi*d_v_vec_0.x - sin_phi*d_v_vec_0.y, sin_phi*d_v_vec_0.x + cos_phi*d_v_vec_0.y, d_v_vec_0.z);
    const float3 u_vec = make_float3(cos_phi*d_u_vec_0.x - sin_phi*d_u_vec_0.y, sin_phi*d_u_vec_0.x + cos_phi*d_u_vec_0.y, d_u_vec_0.z);
    const float3 n_vec = make_float3(cos_phi*d_n_vec_0.x - sin_phi*d_n_vec_0.y, sin_phi*d_n_vec_0.x + cos_phi*d_n_vec_0.y, d_n_vec_0.z);

    const float3 p = make_float3(sourcePos.x, sourcePos.y, sourcePos.z);

    const float3 r = make_float3(u*u_vec.x + v*v_vec.x - n_vec.x, u*u_vec.y + v*v_vec.y - n_vec.y, u*u_vec.z + v*v_vec.z - n_vec.z);

    const float3 dst = make_float3(p.x + D*r.x, p.y + D*r.y, p.z + D*r.z);

    float val;
    if (volumeDimensionOrder == 0)
        val = lineIntegral_Joseph_XYZ(f, N_f, T_f, startVals_f, p, dst);
    else
        val = lineIntegral_Joseph_ZYX(f, N_f, T_f, startVals_f, p, dst);

    if (accum)
        g[uint64(l) * uint64(N_g.z) * uint64(g_rowStride) + uint64(m * N_g.z + n)] += val;
    else
        g[uint64(l) * uint64(N_g.z) * uint64(g_rowStride) + uint64(m * N_g.z + n)] = val;
}

bool project_Joseph_cone(float*& g, float* f, parameters* params, bool data_on_cpu)
{
    return project_Joseph_cone(g, f, params, data_on_cpu, data_on_cpu);
}

bool project_Joseph_cone(float*& g, float* f, parameters* params, bool data_on_cpu, bool volume_on_cpu, bool accumulate)
{
    if (g == NULL || f == NULL || params == NULL || params->allDefined() == false)
        return false;
    ...
    setConstantMemoryGeometryParameters_Joseph(params);

    // Call Kernel
    dim3 dimBlock = setBlockSize(N_g);
    dim3 dimGrid = setGridSize(N_g, dimBlock);

    coneBeamProjectorKernel_Joseph <<< dimGrid, dimBlock >>> (dev_g_out, N_g, T_g, startVal_g, d_data_txt, N_f, T_f, startVal_f, params->sod, params->sdd, params->tau, rFOVsq, dev_phis, params->volumeDimensionOrder, accumulate, g_rowStride);
...
void setConstantMemoryGeometryParameters_Joseph(parameters* params)
{
