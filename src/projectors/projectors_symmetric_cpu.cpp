...
bool CPUinverse_symmetric(float* g, float* f, parameters* params)
{
	...
	int N_phi = int(max(fabs(u_0), fabs(T_u * (params->numCols - 1) + u_0)) / T_u)/2;
	N_phi = N_phi + ((N_phi + 1) % 2);
	float T_phi = 3.1415926535897932385 / float(N_phi);
	...
	float u_unbounded_threshold = params->tau / params->sod;

	float phi_crit = 0.5*PI;
    if (params->tau != 0.0)
        phi_crit = atan(-params->sod/params->tau);
	...
	//float strangeFactor = 2.0*params->tau*params->sod/(params->sod*params->sod - params->tau*params->tau);
	...
	for (int iy = 0; iy < params->numY; iy++)
	{
		...
		{
			float phi = float(iphi) * T_phi + phi_0;
...
bool CPUproject_AbelCone(float* g, float* f, parameters* params)
{
	...
	double u_unbounded_threshold = params->tau / params->sod;
	...
	for (int j = 0; j < params->numRows; j++)
	{
		...
		for (int k = 0; k < params->numCols; k++)
		{
			...
			int rInd_max;
			if (u_unbounded < u_unbounded_threshold)
				rInd_max = N_r_left;
			else
				rInd_max = N_r_right;
			...
			double b_ti = X * params->sod*cos_beta + u_unbounded * params->tau;
            ...
            int rInd_min = int(ceil(r_absoluteMinimum / params->voxelWidth));
...
bool CPUbackproject_AbelCone(float* g, float* f, parameters* params)
{
	...
	for (int j = 0; j < params->numY; j++)
	{
		...
		//*
		double ind_split = (u_unbounded_threshold-u_0) / T_u;
