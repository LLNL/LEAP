...
//########################################################################################################################################################################
//########################################################################################################################################################################
bool doApplyWeights(parameters* params)
{
...
bool CPUbackproject_SF_cone(float* g, float* f, parameters* params, bool setToZero)
{
    ...
    //* this version is faster because it minimizes cache misses
    for (int iphi = 0; iphi < params->numAngles; iphi++)
    {
        float* aProj = &g[uint64(iphi)* uint64(params->numCols*params->numRows)];
...
bool CPUbackproject_SF_cone_kernel_rot(float* aProj, float* xSlice, parameters* params, int ix, int iphi)
{
    float T_v = params->pixelHeight / params->sdd;
    float v_0 = params->v_0() / params->sdd;
    float u_0 = params->u_0() / params->sdd;
    ...
    float v_min = (-0.5-float(params->numRowsExtrapolate))*T_v + v_0;
    float v_max = (float(params->numRows)-0.5+float(params->numRowsExtrapolate))*T_v + v_0;
    ...
    float n_vec_0[3] = {cos_pitch, 0.0, -sin_pitch};
    ...
    float n_vec[3] = {n_vec_0[0]*cos_phi - n_vec_0[1]*sin_phi, n_vec_0[0]*sin_phi + n_vec_0[1]*cos_phi, n_vec_0[2]};
    float u_vec[3] = {u_vec_0[0]*cos_phi - u_vec_0[1]*sin_phi, u_vec_0[0]*sin_phi + u_vec_0[1]*cos_phi, u_vec_0[2]};
    float v_vec[3] = {v_vec_0[0]*cos_phi - v_vec_0[1]*sin_phi, v_vec_0[0]*sin_phi + v_vec_0[1]*cos_phi, v_vec_0[2]};

    float scalars = params->voxelWidth * params->voxelWidth * params->voxelHeight / (T_u*T_v);
    ...
    for (int iy = 0; iy < params->numY; iy++)
    {
        ...
        if (x*x + y*y <= rFOVsq)
        {
            ...
            for (int iz = 0; iz < params->numZ; iz++)
            {
                ...
                float y_phi_minus_x[3] = {params->sod*cos_phi + params->tau*sin_phi - x, params->sod*sin_phi - params->tau*cos_phi - y, params->z_source(iphi) - z};

                float L = y_phi_minus_x[0]*n_vec[0] + y_phi_minus_x[1]*n_vec[1] + y_phi_minus_x[2]*n_vec[2];
                float L_inv = 1.0f / L;
                float u_phi_x = -(y_phi_minus_x[0]*u_vec[0] + y_phi_minus_x[1]*u_vec[1] + y_phi_minus_x[2]*u_vec[2]) * L_inv;
                float v_phi_x = -(y_phi_minus_x[0]*v_vec[0] + y_phi_minus_x[1]*v_vec[1] + y_phi_minus_x[2]*v_vec[2]) * L_inv;

                if (v_min <= v_phi_x && v_phi_x <= v_max)
                {
                    float theWeights = scalars * sqrt(1.0 + u_phi_x*u_phi_x + v_phi_x*v_phi_x) * L_inv * L_inv;

                    float u_ind = max(0.0f, min(float(params->numCols-1), (u_phi_x - u_0) / T_u));
                    float v_ind = max(0.0f, min(float(params->numRows-1), (v_phi_x - v_0) / T_v));
                    int u_lo = int(u_ind);
                    int u_hi = min(params->numCols-1, u_lo+1);
                    float du = u_ind - float(u_lo);
                    int v_lo = int(v_ind);
                    int v_hi = min(params->numRows-1, v_lo+1);
                    float dv = v_ind - float(v_lo);

                    zLine[iz] += ((1.0-dv)*((1.0-du) * aProj[v_lo*params->numCols + u_lo] + du * aProj[v_lo*params->numCols + u_hi])
                                    + dv * ((1.0-du) * aProj[v_hi*params->numCols + u_lo] + du * aProj[v_hi*params->numCols + u_hi])) * theWeights;
                }
            }
        }
    }
...
{
    if (params->tiltAngle != 0.0 || params->pitchAngle != 0.0)
        return CPUbackproject_SF_cone_kernel_rot(aProj, xSlice, params, ix, iphi);
    float T_u, u_0;
    float T_v = params->pixelHeight / params->sdd;
    float v_0 = params->v_0() / params->sdd;
    if (params->detectorType == parameters::CURVED)
