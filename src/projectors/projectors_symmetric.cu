...
__global__ void AbelConeInverseKernel(TEX_DATA g, int4 N_g, float4 T_g, float4 startVals_g, float* f, int4 N_f, float4 T_f, float4 startVals_f, float R, float D, float axisOfSymmetry, float tau, int volumeDimensionOrder)
{
	...
	int N_phi = int(max(fabsf(u_0), fabsf(T_g.z * (N_g.z - 1) + u_0)) * T_u_inv)/2;
	N_phi = N_phi + ((N_phi + 1) % 2);
	const float T_phi = 3.1415926535897932385f / float(N_phi);
	const float R_sq = R * R;

	const float u_unbounded_threshold = tau / R;
	//const float strangeFactor = 2.0f*tau*R/(R_sq - tau*tau);

	float phi_crit = 0.5f*PI;
    if (tau != 0.0f)
        phi_crit = atan(-R/tau);

	float phi_0 = phi_crit;
	if (tau > 0.0f)
        phi_0 = phi_crit - PI;
    else if (tau < 0.0f)
        phi_0 = phi_crit;
	...
	{
		float phi = float(i)*T_phi + phi_0; //- 0.5f * 3.1415926535897932385f;
		...
		//if (r_val*s_val < 0.0f)
		//	s_val *= -1.0f;
		if (s_val < u_unbounded_threshold)
		{
			if (r_val > 0.0f)
				continue;
		}
		...
		//if (r_val*s_val >= 0.0f)
			curVal += theWeight * TEX3D(g, s_arg + 0.5f, v_arg + 0.5f, 0.5f);
		//else
		//	curVal += theWeight * TEX3D(g, s_arg + 0.5f, v_arg + 0.5f, 0.5f);
	}
...
__global__ void AbelParallelInverseKernel(TEX_DATA g, int4 N_g, float4 T_g, float4 startVals_g, float* f, int4 N_f, float4 T_f, float4 startVals_f, float axisOfSymmetry, float tau, int volumeDimensionOrder)
{
	const int i = threadIdx.x + blockIdx.x * blockDim.x;
	const int j = threadIdx.y + blockIdx.y * blockDim.y;
	const int k = threadIdx.z + blockIdx.z * blockDim.z;
	...
	for (int i = 0; i < N_phi; i++)
	{
		...
		//if (r_val * s_val >= 0.0f)
			curVal += TEX3D(g, s_arg + 0.5f, v_arg + 0.5f, 0.5f);
	}
...
__global__ void AbelConeBackprojectorKernel(TEX_DATA g, int4 N_g, float4 T_g, float4 startVals_g, float* f, int4 N_f, float4 T_f, float4 startVals_f, float R, float D, float axisOfSymmetry, float tau, int volumeDimensionOrder)
{
	const int i = threadIdx.x + blockIdx.x * blockDim.x;
	const int j = threadIdx.y + blockIdx.y * blockDim.y;
	const int k = threadIdx.z + blockIdx.z * blockDim.z;
	...
	const float r = fabsf(r_unbounded);
	...
	const float ind_split = (u_unbounded_threshold - startVals_g.z) / T_g.z;
	int iu_max_left, iu_min_right;
	if (fabsf(ind_split - floorf(0.5f + ind_split)) < 1.0e-4)
	{
		iu_max_left = int(floorf(0.5f+ind_split));
		iu_min_right = iu_max_left;
	}
	else
	{
		iu_max_left = int(ind_split);
		iu_min_right = int(ceilf(ind_split));
	}
	...
	{
		// right half
		//iu_min = int(ceilf(-startVals_g.z / T_g.z));
		iu_min = iu_min_right;
		iu_max = N_g.z - 1;
	}
	...
	{
		//const float u = fabsf(T_g.z * float(iu) + startVals_g.z);
		const float u = T_g.z * float(iu) + startVals_g.z;
		...
		if (disc_outer > 0.0f)
		{
			...
			if (disc_inner > 0.0f)
			{
				...
				curVal += sqrt(1.0f + tan_beta * tan_beta + u * u + v_1st_arg * v_1st_arg) * (t_1st_high - t_1st_low) * TEX3D(g, float(iu) + 0.5f, (v_1st_arg - startVals_g.y) * Tv_inv + 0.5f, 0.5f);
				...
				curVal += sqrt(1.0f + tan_beta * tan_beta + u * u + v_2nd_arg * v_2nd_arg) * (t_2nd_high - t_2nd_low) * TEX3D(g, float(iu) + 0.5f, (v_2nd_arg - startVals_g.y) * Tv_inv + 0.5f, 0.5f);
			}
			else
			{
				disc_outer = sqrt(disc_outer);
				const float v_arg = z_slope / (b_ti * a_ti_inv) - tan_beta;
				curVal += sqrt(1.0f + tan_beta * tan_beta + u * u + v_arg * v_arg) * 2.0f * disc_outer * a_ti_inv * TEX3D(g, float(iu) + 0.5f, (v_arg - startVals_g.y) * Tv_inv + 0.5f, 0.5f);
			}
		}
	}
...
__device__ float AbelConeProjectorKernel_left(int4 N_g, float4 T_g, float4 startVals_g, TEX_DATA f, int4 N_f, float4 T_f, float4 startVals_f, float R, float D, float axisOfSymmetry, float tau, int j, int k, int volumeDimensionOrder)
{
	float cos_beta = cos(axisOfSymmetry);
	float sin_beta = sin(axisOfSymmetry);
	if (fabsf(sin_beta) < 1.0e-4)
	...
	const float u_unbounded = float(k) * T_g.z + startVals_g.z;
	const float u = fabsf(u_unbounded);

	const float Rcos_sq_plus_tau_sq = R * R * cos_beta * cos_beta + tau * tau;

	const float X = cos_beta - v * sin_beta;

	const float sec_sq_plus_u_sq = X * X + u * u;
	const float b_ti = X * R * cos_beta + u_unbounded * tau;
	const float a_ti_inv = 1.0f / sec_sq_plus_u_sq;
	//float disc_ti_shift = -(u * R * cos_beta - tau * X) * (u * R * cos_beta - tau * X);
	float disc_ti_shift = b_ti * b_ti - sec_sq_plus_u_sq * Rcos_sq_plus_tau_sq;

	if (fabsf(disc_ti_shift) < 1.0e-8)
		disc_ti_shift = 0.0f;
	if (disc_ti_shift > 0.0f || fabsf(sec_sq_plus_u_sq) < 1.0e-8)
		return 0.0f;
	...
	const int N_r_left = int(floorf(r_center_ind)) + 1;
	//const int N_r_right = N_f.y - N_r_left;
	const float r_max = max((N_f.y - 1) * T_f.y + startVals_f.y, fabsf(startVals_f.y));

	//const int rInd_floor = int(0.5 - startVals_f.y / T_f.y); // first valid index
	//const float r_max = (float)(N_f.y - 1) * T_f.y + startVals_f.y;
	//const float r_min = fabsf((float)(rInd_floor)*T_f.y + startVals_f.y);
	...
	const float r_absoluteMinimum = sqrtf(-disc_ti_shift / sec_sq_plus_u_sq);
    int rInd_min = int(ceilf(r_absoluteMinimum / T_f.y));

	//int rInd_min = (int)ceilf((sqrt(-disc_ti_shift / sec_sq_plus_u_sq)) / T_f.y);
	...
	if (rInd_min >= 1)
	{
		...
		//int rInd_min_minus = max(0, min(N_r - 1, (int)(ceilf(r_absoluteMinimum / T_f.y - 1.0f))));
		int rInd_min_minus = max(0, (int)(ceilf(r_absoluteMinimum / T_f.y - 1.0f)));
		if (r_absoluteMinimum < r_max)
		{
			if (volumeDimensionOrder == 0)
			{
				curVal += max(0.0f, disc_sqrt_prev) * a_ti_inv * TEX3D(f, (b_ti - 0.5f * (disc_sqrt_prev)) * a_ti_inv * z_slope + z_shift + 0.5f, (float)(N_r_left - 1 - rInd_min_minus) + 0.5f, 0.5f);
				curVal += max(0.0f, disc_sqrt_prev) * a_ti_inv * TEX3D(f, (b_ti + 0.5f * (disc_sqrt_prev)) * a_ti_inv * z_slope + z_shift + 0.5f, (float)(N_r_left - 1 - rInd_min_minus) + 0.5f, 0.5f);
			}
			else
			{
				curVal += max(0.0f, disc_sqrt_prev) * a_ti_inv * TEX3D(f, 0.5f, (float)(N_r_left - 1 - rInd_min_minus) + 0.5f, (b_ti - 0.5f * (disc_sqrt_prev)) * a_ti_inv * z_slope + z_shift + 0.5f);
				curVal += max(0.0f, disc_sqrt_prev) * a_ti_inv * TEX3D(f, 0.5f, (float)(N_r_left - 1 - rInd_min_minus) + 0.5f, (b_ti + 0.5f * (disc_sqrt_prev)) * a_ti_inv * z_slope + z_shift + 0.5f);
			}
		}
	}
	...
	for (int ir = rInd_min; ir < N_r_left; ir++)
	{
		...
		if (volumeDimensionOrder == 0)
		{
			curVal += (disc_sqrt_next - disc_sqrt_prev) * a_ti_inv * TEX3D(f, (b_ti - 0.5f * (disc_sqrt_next + disc_sqrt_prev)) * a_ti_inv * z_slope + z_shift + 0.5f, (float)(N_r_left - 1 - ir) + 0.5f, 0.5f);
			curVal += (disc_sqrt_next - disc_sqrt_prev) * a_ti_inv * TEX3D(f, (b_ti + 0.5f * (disc_sqrt_next + disc_sqrt_prev)) * a_ti_inv * z_slope + z_shift + 0.5f, (float)(N_r_left - 1 - ir) + 0.5f, 0.5f);
		}
		else
		{
			curVal += (disc_sqrt_next - disc_sqrt_prev) * a_ti_inv * TEX3D(f, 0.5f, (float)(N_r_left - 1 - ir) + 0.5f, (b_ti - 0.5f * (disc_sqrt_next + disc_sqrt_prev)) * a_ti_inv * z_slope + z_shift + 0.5f);
			curVal += (disc_sqrt_next - disc_sqrt_prev) * a_ti_inv * TEX3D(f, 0.5f, (float)(N_r_left - 1 - ir) + 0.5f, (b_ti + 0.5f * (disc_sqrt_next + disc_sqrt_prev)) * a_ti_inv * z_slope + z_shift + 0.5f);
		}
...
__device__ float AbelConeProjectorKernel_right(int4 N_g, float4 T_g, float4 startVals_g, TEX_DATA f, int4 N_f, float4 T_f, float4 startVals_f, float R, float D, float axisOfSymmetry, float tau, int j, int k, int volumeDimensionOrder)
{
	float cos_beta = cos(axisOfSymmetry);
	float sin_beta = sin(axisOfSymmetry);
	if (fabsf(sin_beta) < 1.0e-4)
	...
	if (rInd_min >= 1)
	{
		...
		if (r_absoluteMinimum < r_max)
		{
			if (volumeDimensionOrder == 0)
			{
				curVal += max(0.0f, disc_sqrt_prev) * a_ti_inv * TEX3D(f, (b_ti - 0.5f * (disc_sqrt_prev)) * a_ti_inv * z_slope + z_shift + 0.5f, (float)(N_r_left + rInd_min_minus) + 0.5f, 0.5f);
				curVal += max(0.0f, disc_sqrt_prev) * a_ti_inv * TEX3D(f, (b_ti + 0.5f * (disc_sqrt_prev)) * a_ti_inv * z_slope + z_shift + 0.5f, (float)(N_r_left + rInd_min_minus) + 0.5f, 0.5f);
			}
			else
			{
				curVal += max(0.0f, disc_sqrt_prev) * a_ti_inv * TEX3D(f, 0.5f, (float)(N_r_left + rInd_min_minus) + 0.5f, (b_ti - 0.5f * (disc_sqrt_prev)) * a_ti_inv * z_slope + z_shift + 0.5f);
				curVal += max(0.0f, disc_sqrt_prev) * a_ti_inv * TEX3D(f, 0.5f, (float)(N_r_left + rInd_min_minus) + 0.5f, (b_ti + 0.5f * (disc_sqrt_prev)) * a_ti_inv * z_slope + z_shift + 0.5f);
			}
		}
	}
	...
	for (int ir = rInd_min; ir < N_r_right; ir++)
	{
		...
		if (volumeDimensionOrder == 0)
		{
			curVal += (disc_sqrt_next - disc_sqrt_prev) * a_ti_inv * TEX3D(f, (b_ti - 0.5f * (disc_sqrt_next + disc_sqrt_prev)) * a_ti_inv * z_slope + z_shift + 0.5f, (float)(N_r_left + ir) + 0.5f, 0.5f);
			curVal += (disc_sqrt_next - disc_sqrt_prev) * a_ti_inv * TEX3D(f, (b_ti + 0.5f * (disc_sqrt_next + disc_sqrt_prev)) * a_ti_inv * z_slope + z_shift + 0.5f, (float)(N_r_left + ir) + 0.5f, 0.5f);
		}
		else
		{
			curVal += (disc_sqrt_next - disc_sqrt_prev) * a_ti_inv * TEX3D(f, 0.5f, (float)(N_r_left + ir) + 0.5f, (b_ti - 0.5f * (disc_sqrt_next + disc_sqrt_prev)) * a_ti_inv * z_slope + z_shift + 0.5f);
			curVal += (disc_sqrt_next - disc_sqrt_prev) * a_ti_inv * TEX3D(f, 0.5f, (float)(N_r_left + ir) + 0.5f, (b_ti + 0.5f * (disc_sqrt_next + disc_sqrt_prev)) * a_ti_inv * z_slope + z_shift + 0.5f);
		}
...
__global__ void AbelConeProjectorKernel(float* g, int4 N_g, float4 T_g, float4 startVals_g, TEX_DATA f, int4 N_f, float4 T_f, float4 startVals_f, float R, float D, float axisOfSymmetry, float tau, int volumeDimensionOrder)
{
	const int i = threadIdx.x + blockIdx.x * blockDim.x;
	const int j = threadIdx.y + blockIdx.y * blockDim.y;
	const int k = threadIdx.z + blockIdx.z * blockDim.z;
	...
	if (i >= N_g.x || j >= N_g.y || k >= N_g.z)
		return;
	else if (float(k) * T_g.z + startVals_g.z < u_unbounded_threshold)
...
__device__ float AbelParallelBeamProjectorKernel_left(int4 N_g, float4 T_g, float4 startVals_g, TEX_DATA f, int4 N_f, float4 T_f, float4 startVals_f, float axisOfSymmetry, int j, int k, int volumeDimensionOrder)
{
	float cos_beta = cos(axisOfSymmetry);
	float sin_beta = sin(axisOfSymmetry);
	if (fabsf(sin_beta) < 1.0e-4)
	...
	const float v = (float)j * T_g.y + startVals_g.y;
	const float u = fabsf((float)k * T_g.z + startVals_g.z);
	...
	int rInd_min = (int)ceilf(u / T_f.y);
	float r_prev = (float)(rInd_min)*T_f.y;
	/*
...
__device__ float AbelParallelBeamProjectorKernel_right(int4 N_g, float4 T_g, float4 startVals_g, TEX_DATA f, int4 N_f, float4 T_f, float4 startVals_f, float axisOfSymmetry, int j, int k, int volumeDimensionOrder)
{
	float cos_beta = cos(axisOfSymmetry);
	float sin_beta = sin(axisOfSymmetry);
	if (fabsf(sin_beta) < 1.0e-4)
...
__global__ void AbelParallelBeamProjectorKernel(float* g, int4 N_g, float4 T_g, float4 startVals_g, TEX_DATA f, int4 N_f, float4 T_f, float4 startVals_f, float axisOfSymmetry, int volumeDimensionOrder)
{
	const int i = threadIdx.x + blockIdx.x * blockDim.x;
	const int j = threadIdx.y + blockIdx.y * blockDim.y;
	const int k = threadIdx.z + blockIdx.z * blockDim.z;
...
__global__ void AbelParallelBeamBackprojectorKernel(TEX_DATA g, int4 N_g, float4 T_g, float4 startVals_g, float* f, int4 N_f, float4 T_f, float4 startVals_f, float axisOfSymmetry, int volumeDimensionOrder)
{
	const int i = threadIdx.x + blockIdx.x * blockDim.x;
	const int j = threadIdx.y + blockIdx.y * blockDim.y;
	const int k = threadIdx.z + blockIdx.z * blockDim.z;
	...
	for (int iu = iu_min; iu <= iu_max; iu++)
	{
		...
		if (disc_outer > 0.0f)
		{
			...
			if (disc_inner > 0.0f)
			{
				...
				curVal += (t_1st_high - t_1st_low) * TEX3D(g, float(iu) + 0.5f, (v_1st_arg - startVals_g.y) * Tv_inv + 0.5f, 0.5f);

				const float t_2nd_low = (b_ti + disc_inner) * a_ti_inv;
				const float t_2nd_high = (b_ti + disc_outer) * a_ti_inv;
				const float v_2nd_arg = -0.5f * (t_2nd_low + t_2nd_high) * tan_beta + z * sec_beta;
				curVal += (t_2nd_high - t_2nd_low) * TEX3D(g, float(iu) + 0.5f, (v_2nd_arg - startVals_g.y) * Tv_inv + 0.5f, 0.5f);
			}
			else
			{
				disc_outer = sqrt(disc_outer);
				const float v_arg = -(b_ti * a_ti_inv) * tan_beta + z * sec_beta;
				curVal += 2.0f * disc_outer * a_ti_inv * TEX3D(g, float(iu) + 0.5f, (v_arg - startVals_g.y) * Tv_inv + 0.5f, 0.5f);
			}
		}
	}
...
bool project_symmetric(float*& g, float* f, parameters* params, bool data_on_cpu)
{
	...
	// Symmetric kernels write g[j*N_g.z + k] (no angle dim). For !data_on_cpu the caller
	// may pass a row-strided device buffer (projectionDataFirstRow/Stride) — pre-offset
	// to the row band the kernel should write into. data_on_cpu uses the contiguous temp
	// dev_g; the strided scatter happens in pullProjectionDataFromGPU.
	float* dev_g_out = dev_g;
	if (!data_on_cpu && params->projectionDataFirstRow > 0)
	...
	TEX_DATA d_data_txt = {};
	TEX_ARRAY d_data_array = loadTexture(d_data_txt, dev_f, N_f, false, useLinearInterpolation, bool(params->volumeDimensionOrder == 1));

	// Call Kernel
	dim3 dimBlock = setBlockSize(N_g);
	dim3 dimGrid = setGridSize(N_g, dimBlock);
	if (params->geometry == parameters::CONE)
		AbelConeProjectorKernel <<< dimGrid, dimBlock >>> (dev_g_out, N_g, T_g, startVal_g, d_data_txt, N_f, T_f, startVal_f, params->sod, params->sdd, params->axisOfSymmetry * PI / 180.0, params->tau, params->volumeDimensionOrder);
	else if (params->geometry == parameters::PARALLEL)
		AbelParallelBeamProjectorKernel <<< dimGrid, dimBlock >>> (dev_g_out, N_g, T_g, startVal_g, d_data_txt, N_f, T_f, startVal_f, params->axisOfSymmetry * PI / 180.0, params->volumeDimensionOrder);
...
bool backproject_symmetric(float* g, float*& f, parameters* params, bool data_on_cpu)
{
	...
	bool useLinearInterpolation = true;
	TEX_DATA d_data_txt = {};
	TEX_ARRAY d_data_array = loadTexture(d_data_txt, dev_g, N_g, false, useLinearInterpolation);

	// Call Kernel
	dim3 dimBlock = setBlockSize(N_f);
	dim3 dimGrid = setGridSize(N_f, dimBlock);
	if (params->geometry == parameters::CONE)
		AbelConeBackprojectorKernel <<< dimGrid, dimBlock >>> (d_data_txt, N_g, T_g, startVal_g, dev_f, N_f, T_f, startVal_f, params->sod, params->sdd, params->axisOfSymmetry * PI / 180.0, params->tau, params->volumeDimensionOrder);
	else if (params->geometry == parameters::PARALLEL)
