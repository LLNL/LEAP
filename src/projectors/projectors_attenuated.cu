...
__global__ void attenuatedWeightedBackprojectorKernel_SF(TEX_DATA g, int4 N_g, float4 T_g, float4 startVals_g, float* f, TEX_DATA mu, int4 N_f, float4 T_f, float4 startVals_f, float rFOVsq, float* phis, int volumeDimensionOrder)
{
    ...
    for (int l = 0; l < N_g.x; l++)
    {
        ...
        val += (-sin_phi * (TEX3D(g, s_ind + s_ind_x_shift, K, L) - TEX3D(g, s_ind - s_ind_x_shift, K, L))
            + cos_phi * (TEX3D(g, s_ind + s_ind_y_shift, K, L) - TEX3D(g, s_ind - s_ind_y_shift, K, L))) * expTerm;
        //*/
        ...
        val += -sin_phi * (TEX3D(g, s_ind + s_ind_x_shift, K, L) * expTerms.w - TEX3D(g, s_ind - s_ind_x_shift, K, L) * expTerms.x)
            + cos_phi * (TEX3D(g, s_ind + s_ind_y_shift, K, L) * expTerms.y - TEX3D(g, s_ind - s_ind_y_shift, K, L) * expTerms.z);
        //*/
    }
...
__global__ void attenuatedBackprojectorKernel_SF(TEX_DATA g, int4 N_g, float4 T_g, float4 startVals_g, float* f, TEX_DATA mu, int4 N_f, float4 T_f, float4 startVals_f, float rFOVsq, float* phis, int volumeDimensionOrder)
{
    ...
    while (l < N_g.x)
    {
        ...
        { //*/
            ...
            val += TEX3D(g, s_ind_A, float(k) + 0.5f, float(l) + 0.5f) * expf(-expTerm);
            l += 1;
        //}
    }
...
__global__ void attenuatedProjectorKernel_SF(float* g, int4 N_g, float4 T_g, float4 startVals_g, TEX_DATA f, TEX_DATA mu, int4 N_f, float4 T_f, float4 startVals_f, float rFOVsq, float* phis, int volumeDimensionOrder, int g_rowStride)
{
    ...
    if (fabs(cos_phi) > fabs(sin_phi))
    {
        ...
        for (int ii = 0; ii < N_f.x; ii++)
        {
            ...
            if (volumeDimensionOrder == 0)
                attenExpCoeff_inc = l_phi * TEX3D(mu, iz, ((u + x * sin_phi) * cos_phi_inv - startVals_f.y) * Tx_inv + 0.5f, ix);
            else
                attenExpCoeff_inc = l_phi * TEX3D(mu, ix, ((u + x * sin_phi) * cos_phi_inv - startVals_f.y) * Tx_inv + 0.5f, iz);
            ...
            if (volumeDimensionOrder == 0)
            {
                g_output += (max(0.0f, min(n_plus_half, s_ind_A + C) - max(n_minus_half, s_ind_A - C)) * TEX3D(f, m, j_min_A, i)
                    + max(0.0f, min(n_plus_half, s_ind_A + ds_ind_dj + C) - max(n_minus_half, s_ind_A + ds_ind_dj - C)) * TEX3D(f, m, j_min_A + 1, i)
                    + max(0.0f, min(n_plus_half, s_ind_A + 2.0f * ds_ind_dj + C) - max(n_minus_half, s_ind_A + 2.0f * ds_ind_dj - C)) * TEX3D(f, m, j_min_A + 2, i))
                    * expf(-attenExpCoeff);
            }
            else
            {
                g_output += (max(0.0f, min(n_plus_half, s_ind_A + C) - max(n_minus_half, s_ind_A - C)) * TEX3D(f, i, j_min_A, m)
                    + max(0.0f, min(n_plus_half, s_ind_A + ds_ind_dj + C) - max(n_minus_half, s_ind_A + ds_ind_dj - C)) * TEX3D(f, i, j_min_A + 1, m)
                    + max(0.0f, min(n_plus_half, s_ind_A + 2.0f * ds_ind_dj + C) - max(n_minus_half, s_ind_A + 2.0f * ds_ind_dj - C)) * TEX3D(f, i, j_min_A + 2, m))
                    * expf(-attenExpCoeff);
            }
        }
    }
    else
    {
        ...
        for (int jj = 0; jj < N_f.y; jj++)
        {
            ...
            float attenExpCoeff_inc;
            if (volumeDimensionOrder == 0)
                attenExpCoeff_inc = l_phi * TEX3D(mu, iz, iy, ((y * cos_phi - u) * sin_phi_inv - startVals_f.x) * Tx_inv + 0.5f);
            else
                attenExpCoeff_inc = l_phi * TEX3D(mu, ((y * cos_phi - u) * sin_phi_inv - startVals_f.x) * Tx_inv + 0.5f, iy, iz);
            ...
            if (volumeDimensionOrder == 0)
            {
                g_output += (max(0.0f, min(n_plus_half, s_ind_A + C) - max(n_minus_half, s_ind_A - C)) * TEX3D(f, m, j, i_min_A)
                    + max(0.0f, min(n_plus_half, s_ind_A + ds_ind_di + C) - max(n_minus_half, s_ind_A + ds_ind_di - C)) * TEX3D(f, m, j, i_min_A + 1)
                    + max(0.0f, min(n_plus_half, s_ind_A + 2.0f * ds_ind_di + C) - max(n_minus_half, s_ind_A + 2.0f * ds_ind_di - C)) * TEX3D(f, m, j, i_min_A + 2))
                    * expf(-attenExpCoeff);
            }
            else
            {
                g_output += (max(0.0f, min(n_plus_half, s_ind_A + C) - max(n_minus_half, s_ind_A - C)) * TEX3D(f, i_min_A, j, m)
                    + max(0.0f, min(n_plus_half, s_ind_A + ds_ind_di + C) - max(n_minus_half, s_ind_A + ds_ind_di - C)) * TEX3D(f, i_min_A + 1, j, m)
                    + max(0.0f, min(n_plus_half, s_ind_A + 2.0f * ds_ind_di + C) - max(n_minus_half, s_ind_A + 2.0f * ds_ind_di - C)) * TEX3D(f, i_min_A + 2, j, m))
                    * expf(-attenExpCoeff);
            }
        }
    }
...
__global__ void cylindricalAttenuatedBackprojectorKernel_SF(TEX_DATA g, int4 N_g, float4 T_g, float4 startVals_g, float* f, const float muCoeff, const float muRadius, int4 N_f, float4 T_f, float4 startVals_f, float rFOVsq, float* phis, int volumeDimensionOrder)
{
    ...
    while (l < N_g.x)
    {
        ...
        val += TEX3D(g, s_ind_A, float(k) + 0.5f, float(l) + 0.5f) * expTerm;
        l += 1;
    }
...
__global__ void cylindricalAttenuatedProjectorKernel_SF(float* g, int4 N_g, float4 T_g, float4 startVals_g, TEX_DATA f, const float muCoeff, const float muRadius, int4 N_f, float4 T_f, float4 startVals_f, float rFOVsq, float* phis, int volumeDimensionOrder, int g_rowStride)
{
    const int l = threadIdx.x + blockIdx.x * blockDim.x;
    const int m = threadIdx.y + blockIdx.y * blockDim.y;
    const int n = threadIdx.z + blockIdx.z * blockDim.z;
        ...
        for (int i = 0; i < N_f.x; i++)
        {
            ...
            if (volumeDimensionOrder == 0)
            {
                g_output += ((weight_0 + weight_1) * TEX3D(f, float(m) + 0.5f, float(j_min_A) + 0.5f + weight_1 / (weight_0 + weight_1), float(i) + 0.5f)
                    + max(0.0f, min(n_plus_half, s_ind_A + 2.0f * ds_ind_dj + C) - max(n_minus_half, s_ind_A + 2.0f * ds_ind_dj - C)) * TEX3D(f, float(m) + 0.5f, float(j_min_A + 2) + 0.5f, float(i) + 0.5f)) * expTerm;
            }
            else
            {
                g_output += ((weight_0 + weight_1) * TEX3D(f, float(i) + 0.5f, float(j_min_A) + 0.5f + weight_1 / (weight_0 + weight_1), float(m) + 0.5f)
                    + max(0.0f, min(n_plus_half, s_ind_A + 2.0f * ds_ind_dj + C) - max(n_minus_half, s_ind_A + 2.0f * ds_ind_dj - C)) * TEX3D(f, float(i) + 0.5f, float(j_min_A + 2) + 0.5f, float(m) + 0.5f)) * expTerm;
            }
        }
    }
    else
        ...
        for (int j = 0; j < N_f.y; j++)
        {
            ...
            if (volumeDimensionOrder == 0)
            {
                g_output += ((weight_0 + weight_1) * TEX3D(f, float(m) + 0.5f, float(j) + 0.5f, float(i_min_A) + 0.5f + weight_1 / (weight_0 + weight_1))
                    + max(0.0f, min(n_plus_half, s_ind_A + 2.0f * ds_ind_di + C) - max(n_minus_half, s_ind_A + 2.0f * ds_ind_di - C)) * TEX3D(f, float(m) + 0.5f, float(j) + 0.5f, float(i_min_A + 2) + 0.5f)) * expTerm;
            }
            else
            {
                g_output += ((weight_0 + weight_1) * TEX3D(f, float(i_min_A) + 0.5f + weight_1 / (weight_0 + weight_1), float(j) + 0.5f, float(m) + 0.5f)
                    + max(0.0f, min(n_plus_half, s_ind_A + 2.0f * ds_ind_di + C) - max(n_minus_half, s_ind_A + 2.0f * ds_ind_di - C)) * TEX3D(f, float(i_min_A + 2) + 0.5f, float(j) + 0.5f, float(m) + 0.5f)) * expTerm;
            }
        }
    }
    g[uint64(l) * uint64(N_g.z) * uint64(g_rowStride) + uint64(m * N_g.z + n)] = l_phi * g_output;
...
bool project_attenuated(float*& g, float* f, parameters* params, bool data_on_cpu)
{
    ...
    // For !data_on_cpu the caller may pass a row-strided device buffer; pre-offset and
    // tell the kernel the true row stride. data_on_cpu uses the contiguous temp dev_g.
    ...
    if (params->mu != NULL)
        attenuatedProjectorKernel_SF <<< dimGrid, dimBlock >>> (dev_g_out, N_g, T_g, startVal_g, d_data_txt, d_mu_txt, N_f, T_f, startVal_f, rFOVsq, dev_phis, params->volumeDimensionOrder, g_rowStride);
    else
        cylindricalAttenuatedProjectorKernel_SF <<< dimGrid, dimBlock >>> (dev_g_out, N_g, T_g, startVal_g, d_data_txt, params->muCoeff, params->muRadius, N_f, T_f, startVal_f, rFOVsq, dev_phis, params->volumeDimensionOrder, g_rowStride);
    ...
    if (params->mu != NULL)
    {
        freeTexture(d_mu_array, d_mu_txt);
    }
    cudaFree(dev_phis);
...
{
    return backproject_attenuated(g, f, params, data_on_cpu, data_on_cpu);
}

bool backproject_attenuated(float* g, float*& f, parameters* params, bool data_on_cpu, bool volume_on_cpu)
{
    if (g == NULL || f == NULL || params == NULL || params->muSpecified() == false || params->allDefined() == false)
        return false;
    if (params->geometry != parameters::PARALLEL)
    ...
    if (params->mu != NULL)
    {
        // mu is volume-shaped; its location follows volume_on_cpu (caller's contract).
        if (volume_on_cpu)
            dev_mu = copyVolumeDataToGPU(params->mu, params, params->whichGPU);
        else
            dev_mu = params->mu;
    ...
    TEX_DATA d_data_txt = {};
    TEX_ARRAY d_data_array = loadTexture(d_data_txt, dev_g, N_g, false, true);
