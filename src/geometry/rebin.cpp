...
float rebin::LagrangeInterpolation2D(float* I, int N_1, int N_2, double x1, double x2, int order)
{
    double ind;
    ...
    M = order;
    if (fabs(ind - double(ind_closest)) < 1.0e-8 || ind_closest < 0 || ind_closest > N_1 - 1)
    {
        ind_lowest_1 = max(0, min(N_1 - 1, ind_closest));
        M_1 = 1;
        h_1[0] = 1.0;
    }
    else
    {
        ...
        {
            // use odd length filter
            M = min(M_default_local - ((M_default_local + 1) % 2), 1 + min(N_1 - 1 - ind_closest, ind_closest));
            if (M % 2 == 0)
                M -= 1;
            ind_lowest = ind_closest - (M - 1) / 2;
        }
        ...
        {
            M = min(M_default_local - (M_default_local % 2), min(N_1 - int(ceil(ind)), 1 + int(ind)));
            if (M % 2 == 1)
                M -= 1;
            ind_lowest = int(ind) - M / 2 + 1;
        }
        ...
        {
            M_1 = 1;
            ind_lowest_1 = max(0, min(N_1 - 1, ind_lowest_1));
            h_1[0] = 1.0;
        }
        else
        {
    ...
    double h_2[8];
    int ind_lowest_2, M_2;

    ind = x2;
    ind_closest = int(floor(0.5 + ind));
    ind_lowest = ind_closest;
    M = order;
    if (fabs(ind - double(ind_closest)) < 1.0e-8 || ind_closest < 0 || ind_closest > N_2 - 1)
    {
        ind_lowest_2 = max(0, min(N_2 - 1, ind_closest));
        M_2 = 1;
        h_2[0] = 1.0;
    }
    else
    {
        ...
        {
            // use odd length filter
            M = min(M_default_local - ((M_default_local + 1) % 2), 1 + min(N_2 - 1 - ind_closest, ind_closest));
            if (M % 2 == 0)
                M -= 1;
            ind_lowest = ind_closest - (M - 1) / 2;
        }
        ...
        {
            M = min(M_default_local - (M_default_local % 2), min(N_2 - int(ceil(ind)), 1 + int(ind)));
            if (M % 2 == 1)
                M -= 1;
            ind_lowest = int(ind) - M / 2 + 1;
        }

        ind_lowest_2 = ind_lowest;
        M_2 = M;

        if (M_2 <= 1)
        {
            M_2 = 1;
            ind_lowest_2 = max(0, min(N_2 - 1, ind_lowest_2));
            h_2[0] = 1.0;
        }
        else
        {
            for (int l = 0; l < M_2; l++)
            {
                ...
                {
                    if (m != l)
                        h *= double(ind - m - ind_lowest_2) / double(l - m);
                }
                h_2[l] = h;
            }
        }
    ...
    for (int l1 = 0; l1 < M_1; l1++)
    {
        ...
        if (0 <= i && i < N_1)
        {
            for (int l2 = 0; l2 < M_2; l2++)
            {
                int k = l2 + ind_lowest_2;
                if (0 <= k && k < N_2)
                {
                    val += h_1[l1] * h_2[l2] * I[i * N_2 + k];
                }
            }
        }
    }
