...
__global__ void calcScaleAndShift(const float* f, float* scale, float* shift, const int3 N, const int r, const float epsilon_sq)
{
    ...
    const float a = varI / (varI + epsilon_sq); // absolute
...
__global__ void guidedFilterSecondStep(float* f, const float* a, const float* b, const int3 N, const int r, const int sliceStart, const int sliceEnd)
{
    const int i = threadIdx.x + blockIdx.x * blockDim.x;
    const int j = threadIdx.y + blockIdx.y * blockDim.y;
    const int k = threadIdx.z + blockIdx.z * blockDim.z;
...
bool guidedFilter(float* f, int N_1, int N_2, int N_3, int r, float epsilon, int numIter, bool data_on_cpu, int whichGPU, int sliceStart, int sliceEnd)
{
    ...
    else
    {
        ...
        {
            calcScaleAndShift <<< dimGrid, dimBlock >>> (dev_f, dev_scale, dev_shift, N, r, epsilon_sq);
            guidedFilterSecondStep <<< dimGrid, dimBlock >>> (dev_f, dev_scale, dev_shift, N, r, sliceStart, sliceEnd);
        }
    }
    ...
    if (data_on_cpu)
    {
        ...
        float* dev_f_shift = &dev_f[uint64(sliceStart) * uint64(N.y) * uint64(N.z)];
        int3 N_crop = make_int3(sliceEnd-sliceStart+1, N_2, N_3);
        pull3DdataFromGPU(&f[uint64(sliceStart) * uint64(N.y) * uint64(N.z)], N_crop, dev_f_shift, whichGPU);
