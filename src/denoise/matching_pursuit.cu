...
__device__ float innerProduct(TEX_DATA f, const float* patches, const int3 voxelIndices,
        ...
        for (int dj = 0; dj < patchSize.y; dj++)
        {
            for (int dk = 0; dk < patchSize.z; dk++)
            {
                ...
                retVal += patches[ind] * //tex2D<float>(patches, whichPatch, ind) *
                    TEX3D(f, voxelIndices.z + dk, voxelIndices.y + dj, voxelIndices.x + di);
                         //TEX3D(f, voxelIndices.x + di, voxelIndices.y + dj, voxelIndices.z + dk);
            }
        }
    }
    return retVal;
...
__device__ void copyVolumePatch(TEX_DATA f, const int3 voxelIndices, const int3 patchSize, float* patch_copy)
{
    for (int di = 0; di < patchSize.x; di++)
    {
        for (int dj = 0; dj < patchSize.y; dj++)
...
__device__ void OMPsinglePatch_basis(TEX_DATA f, const float* patches, const int3 voxelIndices, const int numPatches,
...
__device__ float* OMPsinglePatch(TEX_DATA f, const float* patches, const float* innerProductPairs, const int3 voxelIndices,
...
__global__ void OMP_basis(TEX_DATA f, const float* patches,
...
__global__ void OMP(TEX_DATA f, const float* patches, const float* innerProductPairs,
                    const int3 N, const int numPatches, const int3 patchSize, const float epsilon,
                    const int sparsityThreshold, const int3 patchShift, float* Df, const int3 N_tiles,
                    int* indexMap, float* Rf, float* f_dot_d, float* A_k_inv)
{
...
bool matchingPursuit(float* f, int N_1, int N_2, int N_3, float* dictionary, int numElements, int num1, int num2, int num3, float epsilon, int sparsityThreshold, bool data_on_cpu, int whichGPU)
{
    ...
    TEX_DATA d_data_txt = {};
    TEX_ARRAY d_data_array = loadTexture(d_data_txt, dev_f, N, true, false, false);
