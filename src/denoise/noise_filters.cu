...
__global__ void badPixelCorrectionKernel(float* g, float* badPixelMap, const int3 N, const int windowRadius, const bool ignore_immediate_neighbors)
{
    const int i = threadIdx.x + blockIdx.x * blockDim.x;
    const int j = threadIdx.y + blockIdx.y * blockDim.y;
    const int k = threadIdx.z + blockIdx.z * blockDim.z;
...
__global__ void BlurFilterKernel_txt(TEX_DATA f, float* f_filtered, int3 N, float FWHM, const int sliceStart, const int sliceEnd)
{
    ...
    for (int di = -pixelRadius; di <= pixelRadius; di++)
    {
        ...
        for (int dj = -pixelRadius; dj <= pixelRadius; dj++)
        {
            ...
            for (int dk = -pixelRadius; dk <= pixelRadius; dk++)
            {
                ...
                {
                    //val += theWeight * f[uint64(i_shift) * uint64(N.y * N.z) + uint64(j_shift * N.z + k_shift)];
                    val += theWeight * TEX3D(f, k_shift, j_shift, i_shift);
                    sum += theWeight;
                }
            }
        }
    ...
    if (d_DO_HIGH_PASS)
        f_filtered[uint64(i) * uint64(N.y * N.z) + uint64(j * N.z + k)] = TEX3D(f, k, j, i) - val / sum;
    else
        f_filtered[uint64(i) * uint64(N.y * N.z) + uint64(j * N.z + k)] = val / sum;
}
...
__global__ void BlurFilter2DKernel_txt(TEX_DATA f, float* f_filtered, const int3 N, const float FWHM, const int axis)
{
    ...
    for (int dj = -pixelRadius; dj <= pixelRadius; dj++)
    {
        ...
        for (int dk = -pixelRadius; dk <= pixelRadius; dk++)
        {
            ...
            {
                //val += theWeight * f_slice[uint64(j_shift * N.z + k_shift)];
                val += theWeight * TEX3D(f, k_shift, j_shift, i);
                sum += theWeight;
            }
        }
    }
...
__global__ void BlurFilter1DPeriodicKernel_txt(TEX_DATA f, float* f_filtered, const int3 N, const float FWHM, const int axis)
{
    const int i = threadIdx.x + blockIdx.x * blockDim.x;
    const int j = threadIdx.y + blockIdx.y * blockDim.y;
    const int k = threadIdx.z + blockIdx.z * blockDim.z;
    ...
    if (axis == 0)
    {
        for (int di = -pixelRadius; di <= pixelRadius; di++)
        {
            ...
            if (temp >= N.x)
                temp -= N.x;
            //const int i_shift = (i + di) % N.x;
            //const int i_shift = temp % N.x;
            const int i_shift = max(0, min(temp, N.x - 1));
            ...
            {
                //val += theWeight * f[uint64(i_shift) * uint64(N.y * N.z) + uint64(j * N.z + k)];
                val += theWeight * TEX3D(f, k, j, i_shift);
                sum += theWeight;
            }
        }
    }
    else if (axis == 1)
    {
        for (int dj = -pixelRadius; dj <= pixelRadius; dj++)
        {
            ...
            const int j_shift = max(0, min(temp, N.y - 1));
            //const int j_shift = max(0, min(j + dj, N.y - 1));
            ...
            {
                //val += theWeight * f[uint64(i_shift) * uint64(N.y * N.z) + uint64(j * N.z + k)];
                val += theWeight * TEX3D(f, k, j_shift, i);
                sum += theWeight;
            }
        }
    }
    else //if (axis == 2)
    {
        for (int dk = -pixelRadius; dk <= pixelRadius; dk++)
        {
            ...
            const int k_shift = max(0, min(temp, N.z - 1));
            //const int k_shift = max(0, min(k + dk, N.z - 1));
            ...
            {
                //val += theWeight * f[uint64(i_shift) * uint64(N.y * N.z) + uint64(j * N.z + k)];
                val += theWeight * TEX3D(f, k_shift, j, i);
                sum += theWeight;
            }
        }
    }
...
bool lowOrHighPassFilter_txt(float* f, int N_1, int N_2, int N_3, float FWHM, int numDims, int axis, bool data_on_cpu, int whichGPU, int sliceStart, int sliceEnd, float* f_out)
{
    ...
    if (data_on_cpu)
        d_data_array = loadTexture_from_cpu(d_data_txt, f, N, false, false);
    else
        d_data_array = loadTexture(d_data_txt, f, N, false, false);
    ...
    else
    {
        ...
        {
            // copy dev_Df to dev_f
            cudaMemcpy(f, dev_Df, sizeof(float) * uint64(N.x) * uint64(N.y) * uint64(N.z), cudaMemcpyDeviceToDevice);
            //cudaDeviceSynchronize();
        }
    }
    if (dev_Df != 0 && (f_out == NULL || data_on_cpu == true))
...
bool badPixelCorrection_gpu(float* g, parameters* params, float* badPixelMap, int w, bool data_on_cpu)
{
    ...
    int windowRadius = max(1, min(3, (w - 1) / 2));
    bool ignore_immediate_neighbors = false;
    //if (windowRadius > 1)
    //    ignore_immediate_neighbors = true;

    // Call kernel
    dim3 dimBlock = setBlockSize(N);
    dim3 dimGrid(int(ceil(double(N.x) / double(dimBlock.x))), int(ceil(double(N.y) / double(dimBlock.y))),
        int(ceil(double(N.z) / double(dimBlock.z))));
    badPixelCorrectionKernel <<< dimGrid, dimBlock >>> (dev_g, dev_badPixelMap, N, windowRadius, ignore_immediate_neighbors);
...
bool blurFilter1D_periodic(float* f, int N_1, int N_2, int N_3, float FWHM, int axis, bool data_on_cpu, int whichGPU)
{
    if (f == NULL) return false;
    ...
    dim3 dimGrid(int(ceil(double(N.x) / double(dimBlock.x))), int(ceil(double(N.y) / double(dimBlock.y))),
        int(ceil(double(N.z) / double(dimBlock.z))));
    BlurFilter1DPeriodicKernel_txt <<<dimGrid, dimBlock >>> (d_data_txt, dev_Df, N, FWHM, axis);
