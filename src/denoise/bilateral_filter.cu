...
__global__ void bilateralFilterKernel(float* f, float* f_filtered, const int3 N, const float sigma_d_sq_inv,
                                      const float sigma_i_sq_inv, const int w, const int sliceStart, const int sliceEnd)
{
    // return;
    ...
    if (i < sliceStart || i > sliceEnd) return;
    ...
    const float sigma_i_sq_inv, const int w, const int sliceStart, const int sliceEnd)
{
    ...
    uint64 ind = uint64(i) * uint64(N.z * N.y) + uint64(j * N.z + k);
    //const float curVal = f[ind];
    const float curVal = TEX3D(f, k, j, i);
    ...
    for (int di = di_min; di <= di_max; di++)
    {
        ...
        for (int dj = dj_min; dj <= dj_max; dj++)
        {
            ...
            for (int dk = dk_min; dk <= dk_max; dk++)
            {
                //const float neighborVal = f_slice[uint64((j + dj) * N.z + (k + dk))];
                //const float neighborVal = f_line[k + dk];
                const float neighborVal = TEX3D(f, k+dk, j+dj, i+di);
    ...
    for (int di = di_min; di <= di_max; di++)
    {
        ...
        for (int dj = dj_min; dj <= dj_max; dj++)
        {
            ...
            for (int dk = dk_min; dk <= dk_max; dk++)
            {
                //const float neighborVal = Bf[uint64(i + di) * uint64(N.z * N.y) + uint64((j + dj) * N.z + (k + dk))];
                //const float neighborVal = Bf_line[k + dk];
                const float neighborVal = TEX3D(Bf, k+dk, j+dj, i+di);
...
bool bilateralFilter(float* f, int N_1, int N_2, int N_3, float spatialFWHM, float intensityFWHM, bool data_on_cpu, int whichGPU, int sliceStart, int sliceEnd)
{
    ...
    if (useTexture)
    {
        ...
        bilateralFilterKernel_txt <<< dimGrid, dimBlock >>> (f_data_txt, dev_Df, N, sigma_d_sq_inv, sigma_i_sq_inv, w, sliceStart, sliceEnd);
    }
    else
    {
        bilateralFilterKernel <<< dimGrid, dimBlock >>> (dev_f, dev_Df, N, sigma_d_sq_inv, sigma_i_sq_inv, w, sliceStart, sliceEnd);
    }
    ...
    if (data_on_cpu)
    {
        ...
        float* dev_Df_shift = &dev_Df[uint64(sliceStart) * uint64(N.y) * uint64(N.z)];
        int3 N_crop = make_int3(sliceEnd-sliceStart+1, N_2, N_3);
        pull3DdataFromGPU(&f[uint64(sliceStart) * uint64(N.y) * uint64(N.z)], N_crop, dev_Df_shift, whichGPU);
...
//*
bool priorBilateralFilter(float* f, int N_1, int N_2, int N_3, float spatialFWHM, float intensityFWHM, float* Bf, bool data_on_cpu, int whichGPU, int sliceStart, int sliceEnd)
{
    if (Bf == NULL)
        return bilateralFilter(f, N_1, N_2, N_3, spatialFWHM, intensityFWHM, data_on_cpu, whichGPU, sliceStart, sliceEnd);
    if (f == NULL) return false;
    ...
    TEX_DATA f_data_txt = {};
    TEX_ARRAY f_data_array = NULL;
    TEX_DATA Bf_data_txt = {};
    TEX_ARRAY Bf_data_array = NULL;
    if (useTexture)
    {
        f_data_array = loadTexture(f_data_txt, dev_f, N, false, false);
        Bf_data_array = loadTexture(Bf_data_txt, dev_Bf, N, false, false);
        scaledBilateralFilterKernel_txt <<< dimGrid, dimBlock >>> (f_data_txt, Bf_data_txt, dev_Df, N, sigma_d_sq_inv, sigma_i_sq_inv, w, sliceStart, sliceEnd);
    }
    else
    {
        scaledBilateralFilterKernel <<< dimGrid, dimBlock >>> (dev_f, dev_Bf, dev_Df, N, sigma_d_sq_inv, sigma_i_sq_inv, w, sliceStart, sliceEnd);
    }
    cudaDeviceSynchronize();
    ...
    {
        freeTexture(f_data_array, f_data_txt);
        freeTexture(Bf_data_array, Bf_data_txt);
    }
    if (data_on_cpu)
    {
        // pull result off GPU
...
bool scaledBilateralFilter(float* f, int N_1, int N_2, int N_3, float spatialFWHM, float intensityFWHM, float scale, bool data_on_cpu, int whichGPU, int sliceStart, int sliceEnd)
{
    if (scale <= 1.0)
        return bilateralFilter(f, N_1, N_2, N_3, spatialFWHM, intensityFWHM, data_on_cpu, whichGPU, sliceStart, sliceEnd);
    if (f == NULL) return false;
