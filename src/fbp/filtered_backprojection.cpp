...
{
	//*
	bool do_antisymmetric = false;
	if (params->tau == 0.0)
		do_antisymmetric = true;
	//*/
	//bool do_antisymmetric = true;
...
bool filteredBackprojection::filterProjections(float* g, float* g_out, parameters* params, bool data_on_cpu)
{
	...
	if (params->whichGPU < 0 || data_on_cpu == false)
	{
		...
		if (params->geometry == parameters::CONE && params->helicalPitch != 0.0)
		{
			...
			if ((params->inconsistencyReconstruction == true && params->inconsistencyReconstruction_has_adequate_angular_range() == true) || params->doDBP)
			{
				if (data_on_cpu)
				...
				if (params->doDBP)
					applyDBPviewWeights(g, params, data_on_cpu);
				return true;
			}
			else
		...
		else
		{
			...
			else if ((params->inconsistencyReconstruction == true && params->inconsistencyReconstruction_has_adequate_angular_range() == true) || params->doDBP)
			{
				float optional_scalar = FBPscalar(params);
				float sample_shift = 0.0;
				if (params->doDBP)
				{
					optional_scalar *= 1.0 / params->sod;
					sample_shift = 1.0;
				}
				if (params->whichGPU < 0)
					ray_derivative_cpu(g, params, sample_shift, optional_scalar);
				else
					ray_derivative(g, params, data_on_cpu, optional_scalar, sample_shift);
				if (params->doDBP)
					applyDBPviewWeights(g, params, data_on_cpu);
			}
			else
			...
			else if ((params->inconsistencyReconstruction == true && params->inconsistencyReconstruction_has_adequate_angular_range() == true) || params->doDBP)
			{
				...
				ray_derivative_cpu(g, params, sample_shift, optional_scalar);
				if (params->doDBP)
					applyDBPviewWeights_CPU(g, params);
			}
			else
				rampFilterProjections(g, params, data_on_cpu, FBPscalar(params));
			if (params->muCoeff != 0.0)
...
bool filteredBackprojection::execute(float* g, float* f, parameters* params, bool data_on_cpu)
{
	...
	bool volume_on_cpu = data_on_cpu && !has_unified_memory;
	return execute(g, f, params, data_on_cpu, volume_on_cpu);
}
...
bool filteredBackprojection::execute(float* g, float* f, parameters* params, bool data_on_cpu, bool volume_on_cpu, bool accumulate)
{
	...
	if (params->muSpecified())
		return execute_attenuated(g, f, params, data_on_cpu, volume_on_cpu);
...
{
	return execute_attenuated(g, f, params, data_on_cpu, data_on_cpu);
}

bool filteredBackprojection::execute_attenuated(float* g, float* f, parameters* params, bool data_on_cpu, bool volume_on_cpu)
{
	if (params->mu != NULL)
		return execute_Novikov(g, f, params, data_on_cpu, volume_on_cpu);
	...
	if (params->whichGPU < 0 || data_on_cpu == false)
	{
		...
		params->muCoeff *= -1.0;
		bool retVal = proj.backproject(g, f, params, false, volume_on_cpu);
		params->muCoeff *= -1.0;
		return retVal;
	}
	else
	{
		...
		retVal = execute_attenuated(dev_g, dev_f, params, false, false);
...
{
	return execute_Novikov(g, f, params, data_on_cpu, data_on_cpu);
}

bool filteredBackprojection::execute_Novikov(float* g, float* f, parameters* params, bool data_on_cpu, bool volume_on_cpu)
{
#ifndef __USE_CPU
	if (params->geometry != parameters::PARALLEL)
	{
	...
	if ((data_on_cpu || volume_on_cpu) && params->hasSufficientGPUmemory() == false)
	{
		printf("Error: insufficient GPU memory\n");
		return false;
	}
	...
	if (volume_on_cpu)
	{
		// mu and f are volume-shaped; copy mu, allocate dev_f when caller has them on the host.
		dev_mu = copyVolumeDataToGPU(params->mu, params, params->whichGPU);
		if (dev_mu == 0)
		{
			fprintf(stderr, "cudaMalloc(volume) failed!\n");
	...
	proj.weightedBackproject(dev_g, dev_f, params, false, false);
	params->mu = params_mu_save;
