...
bool conv1D_cpu(float*& g, parameters* params, float scalar, int whichFilter)
{
    ...
    {
        for (int iRow = 0; iRow < params->numRows; iRow+=2)
        {
            shape_t shape{size_t(N_H)};
            stride_t stridef(shape.size());

            shape_t axes;
            axes.push_back(0);
            vector<complex<float>> paddedArray(N_H);
            stridef[0] = sizeof(complex<float>);

            float* proj_line = &g[uint64(iphi) * uint64(params->numRows * params->numCols) + uint64(iRow * params->numCols)];
            float* proj_line2 = proj_line; 
            if (iRow+1 < params->numRows)
                proj_line2  = &g[uint64(iphi) * uint64(params->numRows * params->numCols) + uint64((iRow+1) * params->numCols)];

            // Put in padded array
            for (int i = 0; i < N_H; i++)
            {
                if (i < params->numCols)
                    paddedArray[i] = complex<float>(proj_line[i], proj_line2[i]);
                else
                    paddedArray[i] = 0.0;
            }
            ...
            if (numExtrapolate > 0)
            {
                ...
                float leftVal2 = proj_line2[0];
                float rightVal2 = proj_line2[params->numCols - 1];
                for (int i = params->numCols; i < params->numCols + numExtrapolate; i++)
                    paddedArray[i] = complex<float>(rightVal, rightVal2);
                for (int i = N_H - numExtrapolate; i < N_H; i++)
                    paddedArray[i] = complex<float>(leftVal, leftVal2);
            }
            ...
            // Do FFT
            //fft(data);
            auto resf = paddedArray;
            c2c(shape, stridef, stridef, axes, FORWARD,
                paddedArray.data(), resf.data(), 1.f);
            ...
            c2c(shape, stridef, stridef, axes, BACKWARD,
                resf.data(), paddedArray.data(), 1.f);

            // Copy back to array
            for (int i = 0; i < params->numCols; i++)
            {
                proj_line[i] = real(paddedArray[i]);
                proj_line2[i] = imag(paddedArray[i]);
            }
        }
        //delete[] paddedArray;
    }
...
float* rampFilterFrequencyResponseMagnitude_cpu(int N, parameters* params)
{
    ...
    shape_t axes;
    axes.push_back(0);
    vector<complex<float>> filterData(N);
    stridef[0] = sizeof(complex<float>);

    for (int i = 0; i < N; i++)
        filterData[i] = h[i];

    auto resf = filterData;
    c2c(shape, stridef, stridef, axes, FORWARD,
        filterData.data(), resf.data(), 1.f);
...
double rampImpulseResponse(int N, double T, int n, int rampID, bool shifted)
{
    double retVal = 0.0;
...
float* rampImpulseResponse_f(int N, float T, parameters* params, bool centered)
{
    if (centered)
    {
        float* h_f = (float*)malloc(sizeof(float) * N);
        for (int i = 0; i < N; i++)
            h_f[i] = rampImpulseResponse(N, T, i, params->rampID, false);
        return h_f;
    }
    else
    {
        double* h = rampImpulseResponse(N, double(T), params);
        float* h_f = (float*)malloc(sizeof(float) * N);
        for (int i = 0; i < N; i++)
            h_f[i] = h[i];
        free(h);
...
Complex* HilbertTransformFrequencyResponse_cpu(int N, parameters* params)
{
    ...
    for (int i = 0; i < N; i++)
    {
        H_comp[i] = resf[i];// / Complex(N);
        H_comp[i] = H_comp[i] / Complex(N); 
    }
...
bool ray_derivative_cpu(float* g, parameters* params, float sampleShift, float scalar)
{
    ...
    int N[3] = {params->numAngles, params->numRows, params->numCols};
    int shift = 0;
    if (sampleShift > 0.0)
        shift = 1;
    ...
    if (params->doDBP == false)
        rampID = 2;

    //*
    finite_difference(g, N, rampID, shift, NULL, c);
    params->colShiftFromFilter += -0.5*float(shift);
    //*/

    /*
    int N_h = 25;
    float* h = rampImpulseResponse_f(N_h, 1.0, params, true);
    for (int i = 0; i < N_h; i++)
        printf("%f\n", h[i]);
    bool axis[3] = {false, false, true};
    antialias_filter(g, N, 3.0, 2, h, N_h, axis);
    free(h);
    //*/
    ...
    for (int i = 0; i < params->numAngles; i++)
    {
        ...
        for (int j = 0; j < params->numRows; j++)
        {
            ...
            memcpy(temp, aLine, sizeof(float)*params->numCols);
            //for (int k = 0; k < params->numCols; k++)
            //    temp[k] = aLine[k];
