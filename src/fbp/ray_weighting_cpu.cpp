...
bool applyDBPviewWeights(float* g, parameters* params, bool data_on_cpu)
{
	#ifndef __USE_CPU
	if (params->whichGPU < 0)
		return applyDBPviewWeights_CPU(g, params);
	else
		return applyDBPviewWeights_GPU(g, params, data_on_cpu);
	#else
		return applyDBPviewWeights_CPU(g, params);
	#endif
}
...
float* setViewWeights(parameters* params)
{
	if (params->helicalPitch != 0.0)
	{
		/*
		if (params->doDBP)
			return setDBPviewWeights(params);
		else
			return NULL;
		//*/
		return NULL;
	...
	else
	{
		...
		/*
		if (params->doDBP)
			w = setDBPviewWeights(params);
		else if (params->inconsistencyReconstruction == false)
			w = setParkerWeights(params);
...
float* setParkerWeights(parameters* params)
{
	if (params->numAngles == 1)
		...
		return NULL;
	else if (params->less_than_full_scan() == false) //if (params->angularRange >= 359.9999)
		return NULL;
	else if (params->inconsistencyReconstruction == true && params->angularRange >= 350.0)
		return NULL;
...
float* setDBPviewWeights(parameters* params)
{
	...
	if (params->geometry == parameters::CONE || params->geometry == parameters::FAN || params->geometry == parameters::MODULAR)
	{
		...
		for (int i = 0; i < params->numAngles; i++)
		{
			for (int j = 0; j < params->numCols; j++)
			{
				float phi = params->phis[i] - atan(params->u(j));
				float theWeight;
				/*
				if (sin(phi) > 0.0 || int(phi/PI) % 2 == 0)
					theWeight = 1.0;
				else
					theWeight = -1.0;
				//*/
...
float* setRedundantAndNonEquispacedViewWeights(parameters* params, float* w)
{
	...
	if (params->less_than_full_scan() == false && params->helicalPitch == 0.0)
	{
		float c = 0.5;
...
{
	float* w = setDBPviewWeights(params);
	if (w == NULL)
		return true;
	else
	{
		...
		for (int iphi = 0; iphi < params->numAngles; iphi++)
		{
			for (int iv = 0; iv < params->numRows; iv++)
			{
				for (int iu = 0; iu < params->numCols; iu++)
				{
					float theWeight = w[iphi * params->numCols + iu];
					g[uint64(iphi) * uint64(params->numRows * params->numCols) + uint64(iv * params->numCols + iu)] *= theWeight;
				}
			}
		}
			...
			for (int iv = 0; iv < params->numRows; iv++)
			{
				for (int iu = 0; iu < params->numCols; iu++)
				{
					...
					if (w != NULL)
						theWeight *= w[iv * params->numCols + iu];
					if (w_parker != NULL)
						theWeight *= w_parker[iphi * params->numCols + iu];
					g[uint64(iphi) * uint64(params->numRows * params->numCols) + uint64(iv * params->numCols + iu)] *= theWeight;
				}
			}
		}
