...
__global__ void convertARTtoERTkernel(float* __restrict__ g, const float muCoeff, const float muRadius, const float T_u, const float u_0, int3 N)
{
...
__global__ void applyViewDependentPolarWeightsKernel(float* __restrict__ g, const float* __restrict__ w_polar, const int3 N, const bool doInverse)
{
	const int i = threadIdx.x + blockIdx.x * blockDim.x;
	const int j = threadIdx.y + blockIdx.y * blockDim.y;
	const int k = threadIdx.z + blockIdx.z * blockDim.z;
...
__global__ void applyWeightsKernel(float* __restrict__ g, const float* __restrict__ w_view, const float* __restrict__ w_ray, int3 N)
{
	const int i = threadIdx.x + blockIdx.x * blockDim.x;
	const int j = threadIdx.y + blockIdx.y * blockDim.y;
	const int k = threadIdx.z + blockIdx.z * blockDim.z;
...
bool applyDBPviewWeights_GPU(float* g, parameters* params, bool data_on_cpu)
{
	float* w_ray = NULL;
	float* w_view = setDBPviewWeights(params); // numAngles X numCols

	if (w_view == NULL)
		return true;
	else
	{
		LOG(logDEBUG, "ray_weighting", "applyDBPviewWeights_GPU") << "GPU " << params->whichGPU << ": start" << std::endl;
		...
		if (w_view != NULL)
		{
			if (cudaSuccess != cudaMalloc((void**)&dev_w_view, params->numAngles * params->numCols * sizeof(float)))
				fprintf(stderr, "Error: applyDBPviewWeights_GPU: cudaMalloc failed!\n");
			if (cudaSuccess != cudaMemcpy(dev_w_view, w_view, params->numAngles * params->numCols * sizeof(float), cudaMemcpyHostToDevice))
				fprintf(stderr, "Error: applyDBPviewWeights_GPU: cudaMemcpy failed!\n");
		}
		float* dev_w_ray = 0;
		if (w_ray != NULL)
		{
...
bool applyPostRampFilterWeights_GPU(float* g, parameters* params, bool data_on_cpu)
{
	...
	if (params->inconsistencyReconstruction)
		w_parker = setParkerWeights(params);
	if (w_ray == NULL && w_parker == NULL)
		return true;
	else
	{
		...
		if (w_parker != NULL)
		{
			if (cudaSuccess != cudaMalloc((void**)&dev_w_view, params->numAngles * params->numCols * sizeof(float)))
				fprintf(stderr, "Error: applyPostRampFilterWeights_GPU: cudaMalloc failed!\n");
			if (cudaSuccess != cudaMemcpy(dev_w_view, w_parker, params->numAngles * params->numCols * sizeof(float), cudaMemcpyHostToDevice))
				fprintf(stderr, "Error: applyPostRampFilterWeights_GPU: cudaMemcpy failed!\n");
		}
