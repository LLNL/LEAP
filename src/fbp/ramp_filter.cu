...
float smooth_rect(float x, float w, float k)
{
    return 0.5*tanh(k*(x+w))-0.5*tanh(k*(x-w));
}

__global__ void zeroPadForOffsetScanKernel(const float* __restrict__ g, float* __restrict__ g_pad, const int3 N, const int N_add, const bool padOnLeft, const float* __restrict__ offsetScanWeights)
{
...
__global__ void multiplyRampFilterKernel(cufftComplex* __restrict__ G, const float* __restrict__ H, int3 N)
{
    /*
    int j = threadIdx.x;
    int i = blockIdx.x;
...
__global__ void multiplyComplexFilterKernel(cufftComplex* __restrict__ G, const cufftComplex* __restrict__ H, int3 N)
{
    /*
    int j = threadIdx.x;
    int i = blockIdx.x;
...
__global__ void multiply2DRampFilterKernel(cufftComplex* __restrict__ F, const float* __restrict__ H, int3 N)
{
    /*
    // int k = threadIdx.x;
...
__global__ void explicit_convolution(TEX_DATA g, float* __restrict__ filtered_data, TEX_DATA h, int3 N, int N_filter)
{
    ...
    {
        const float x0 = TEX3D(g, j + 0, m, l);
        const float x1 = TEX3D(g, j + 1, m, l);
        const float x2 = TEX3D(g, j + 2, m, l);
        const float x3 = TEX3D(g, j + 3, m, l);
        const float x4 = TEX3D(g, j + 4, m, l);
        const float x5 = TEX3D(g, j + 5, m, l);
        const float x6 = TEX3D(g, j + 6, m, l);
        const float x7 = TEX3D(g, j + 7, m, l);

        const int n_minus_j_plus_N = n - j + N.z;
        for (int s = 0; s < NUM_RAYS_PER_THREAD; s++)
        {
            ys[s] += TEX1D(h, n_minus_j_plus_N + s - 0) * x0 + TEX1D(h, n_minus_j_plus_N + s - 1) * x1
                  +  TEX1D(h, n_minus_j_plus_N + s - 2) * x2 + TEX1D(h, n_minus_j_plus_N + s - 3) * x3
                  +  TEX1D(h, n_minus_j_plus_N + s - 4) * x4 + TEX1D(h, n_minus_j_plus_N + s - 5) * x5
                  +  TEX1D(h, n_minus_j_plus_N + s - 6) * x6 + TEX1D(h, n_minus_j_plus_N + s - 7) * x7;
        }
    }
...
__global__ void setPaddedDataFor2DFilter_reverse(float* __restrict__ g, const float* __restrict__ g_pad, const int numRows, const int numCols, const int N_H1, const int N_H2, const float minValue, const bool isAttenuationData)
{
    ...
    if (isAttenuationData)
        g[j * numCols + i] = -logf(max(minValue, expf(-g[j * numCols + i]) + g_pad[j * N_H2 + i] / float(N_H1 * N_H2)));
    else
        g[j * numCols + i] = max(minValue, g[j * numCols + i] + g_pad[j * N_H2 + i] / float(N_H1 * N_H2));
}

__global__ void setPaddedDataFor2DFilter(const float* __restrict__ g, float* __restrict__ g_pad, const int numRows, const int numCols, const int N_H1, const int N_H2, const bool isAttenuationData)
{
    const int j = threadIdx.x + blockIdx.x * blockDim.x; // rows
    const int i = threadIdx.y + blockIdx.y * blockDim.y; // columns
...
__global__ void Laplacian_kernel(const float* __restrict__ g, float* __restrict__ Dg, const int4 N, const float4 T, const float4 startVal, const int numDims, const bool smooth, const float scalar)
{
    const int l = threadIdx.x + blockIdx.x * blockDim.x;
    const int m = threadIdx.y + blockIdx.y * blockDim.y;
    ...
    float diff = 0.0f;
    const float* proj = &g[uint64(l) * uint64(N.z * N.y)];
    if (smooth)
    {
        if (N.z >= 3)
        {
...
__global__ void ray_derivative_kernel_inplace_high_order(float* __restrict__ g, const int4 N, const float* __restrict__ h, const int N_h)
{
    const int l = threadIdx.x + blockIdx.x * blockDim.x;
    const int m = threadIdx.y + blockIdx.y * blockDim.y;
    if (l >= N.x || m >= N.y)
    ...
    for (int n = ind_min; n <= ind_max; n++)
        buffer[n-ind_min] = aLine[max(0, min(n, N.z-1))];

    for (int n = 0; n < N.z; n++)
    {
        float accum = 0.0f;
        #pragma unroll
        for (int k = ind_min; k <= ind_max; ++k)
            accum += h[k - ind_min] * buffer[(n + k - ind_min) & 15]; //n + k - ind_min: n to n+ind_max-ind_min=n+N_h-1

        aLine[n] = accum;

        buffer[(n+1+N_h-1) & 15] = aLine[min(N.z-1, n+1+ind_max)];
    }
}

__global__ void ray_derivative_kernel_inplace(float* __restrict__ g, const int4 N, const float4 T, const float4 startVal, const float scalar, const float sampleShift)
{
    const int l = threadIdx.x + blockIdx.x * blockDim.x;
    const int m = threadIdx.y + blockIdx.y * blockDim.y;
    if (l >= N.x || m >= N.y)
    ...
    if (sampleShift == 0.0f)
    {
        // central difference
        float prev = aLine[0];
        const float combinedScaling = scalar * 0.5f / T.z;
        aLine[0] = (aLine[1] - aLine[0]) * combinedScaling;

        for (int n = 1; n < N.z-1; n++)
        {
            const float cur = aLine[n];
            aLine[n] = (aLine[n+1] - prev) * combinedScaling;
            prev = cur;
        }
        aLine[N.z-1] = (aLine[N.z-1] - prev) * combinedScaling;
        //aLine[0] = aLine[1];
        //aLine[N.z-1] = aLine[N.z-2];
    }
    else if (sampleShift > 0.0f)
    {
        // forward difference
        const float combinedScaling = scalar / T.z;
        aLine[0] = (aLine[1] - aLine[0]) * combinedScaling;

        for (int n = 1; n < N.z-1; n++)
            aLine[n] = (aLine[n+1] - aLine[n]) * combinedScaling;
        aLine[N.z-1] = 0.0f;
    }
    else
    {
        ...
        for (int n = N.z-1; n >= 1; n--)
            aLine[n] = (aLine[n] - aLine[n-1]) * combinedScaling;
        aLine[0] = 0.0f;
    }

}

__global__ void ray_derivative_kernel(float* __restrict__ g, float* __restrict__ Dg, const int4 N, const float4 T, const float4 startVal, const float scalar, const float sampleShift)
{
    const int l = threadIdx.x + blockIdx.x * blockDim.x;
    const int m = threadIdx.y + blockIdx.y * blockDim.y;
    const int n = threadIdx.z + blockIdx.z * blockDim.z;
...
__global__ void deriv_helical_NHDLH_curved(TEX_DATA g, float* __restrict__ Dg, const int4 N, const float4 T, const float4 startVal, const float R, const float D, const float tau, const float helicalPitch, const float epsilon, const float* __restrict__ phis, const int iphi_offset)
{
    ...
    const float u = max(1 * T.z + startVal.z + 0.5f * T.z, min((N.z-2) * T.z + startVal.z + 0.5f * T.z, n * T.z + startVal.z + 0.5f * T.z));
    ...
    const float term1 = TEX3D(g, u_arg, v_arg, l0 + 0.5f);
    ...
    u_arg = one_over_T_u * atan((sin_phi_shift*B0 - cos_phi_shift*B1) / (cos_phi_shift*B0 + sin_phi_shift*B1)) - u_shift;
    v_arg = one_over_T_v * B2*rsqrt(B0*B0 + B1*B1) - v_shift;
    const float term2 = TEX3D(g, u_arg, v_arg, (float)l_next + 0.5f);
    ...
    u_arg = one_over_T_u * atan((sin_phi_shift*B0 - cos_phi_shift*B1) / (cos_phi_shift*B0 + sin_phi_shift*B1)) - u_shift;
    v_arg = one_over_T_v * B2*rsqrt(B0*B0 + B1*B1) - v_shift;
    const float term3 = TEX3D(g, u_arg, v_arg, l0 + 0.5f);
    ...
    u_arg = one_over_T_u * atan((sin_phi_shift*B0 - cos_phi_shift*B1) / (cos_phi_shift*B0 + sin_phi_shift*B1)) - u_shift;
    v_arg = one_over_T_v * B2*rsqrt(B0*B0 + B1*B1) - v_shift;
    const float term4 = TEX3D(g, u_arg, v_arg, (float)l_prev + 0.5f);
...
__global__ void deriv_helical_NHDLH_flat_rot(TEX_DATA g, float* __restrict__ Dg, const int4 N, const float4 T, const float4 startVal, const float R, const float D, const float tau, const float helicalPitch, const float epsilon, const float* __restrict__ phis, const int iphi_offset, const float T_ave)
{
    const int l = threadIdx.x + blockIdx.x * blockDim.x + iphi_offset;
    const int m = threadIdx.y + blockIdx.y * blockDim.y;
    const int n = threadIdx.z + blockIdx.z * blockDim.z;
    ...
    //const float v_0 = m * T.y + startVal.y;
    //const float u_0 = n * T.z + startVal.z + 0.5f*T.z;
    //const float v = u_0*sin(PI/180.0) + v_0*cos(PI/180.0);
    //const float u = u_0*cos(PI/180.0) - v_0*sin(PI/180.0);

    const float v = m * T.y + startVal.y;
    //const float u = n * T.z + startVal.z + 0.5f*T.z;
    const float u = max(1 * T.z + startVal.z + 0.5f * T.z, min((N.z-2) * T.z + startVal.z + 0.5f * T.z, n * T.z + startVal.z + 0.5f * T.z));

    const float3 Theta_0 = make_float3(-d_n_vec_0.x + u*d_u_vec_0.x + v*d_v_vec_0.x,
                                       -d_n_vec_0.y + u*d_u_vec_0.y + v*d_v_vec_0.y,
                                       -d_n_vec_0.z + u*d_u_vec_0.z + v*d_v_vec_0.z);
    const float lineLength = -(R * Theta_0.x - tau * Theta_0.y) / (1.0f + u*u + v*v - (Theta_0.z*Theta_0.z));
    ...
    const float view_spacing_correction = T_phi / T_ave;
    ...
    const float3 Theta_phi = make_float3(Theta_0.x*cos_phi - Theta_0.y*sin_phi, Theta_0.x*sin_phi + Theta_0.y*cos_phi, Theta_0.z);
    ...
    cos_phi_shift = cos_phi;
    sin_phi_shift = sin_phi;
    B0 = R*(cos_phi_epsilon-cos_phi_shift) + tau*(sin_phi_epsilon-sin_phi_shift) + lineLength*Theta_phi.x;
    B1 = R*(sin_phi_epsilon-sin_phi_shift) - tau*(cos_phi_epsilon-cos_phi_shift) + lineLength*Theta_phi.y;
    B2 = helicalPitch *(epsilon-(float)shiftDirection)*T_phi + lineLength*Theta_phi.z;
    ...
    one_over_neg_B_dot_theta = -1.0f / (B0*(d_n_vec_0.x*cos_phi_shift - d_n_vec_0.y*sin_phi_shift) + B1*(d_n_vec_0.x*sin_phi_shift + d_n_vec_0.y*cos_phi_shift) + B2*(d_n_vec_0.z));
    u_arg = one_over_T_u * (B0*(d_u_vec_0.x*cos_phi_shift - d_u_vec_0.y*sin_phi_shift) + B1*(d_u_vec_0.x*sin_phi_shift + d_u_vec_0.y*cos_phi_shift) + B2*(d_u_vec_0.z)) * one_over_neg_B_dot_theta - u_shift;
    v_arg = one_over_T_v * (B0*(d_v_vec_0.x*cos_phi_shift - d_v_vec_0.y*sin_phi_shift) + B1*(d_v_vec_0.x*sin_phi_shift + d_v_vec_0.y*cos_phi_shift) + B2*(d_v_vec_0.z)) * one_over_neg_B_dot_theta - v_shift;
    const float term1 = TEX3D(g, u_arg, v_arg, l0+0.5f);
    ...
    Dg[uint64(l) * uint64(N.z * N.y) + uint64(m * N.z + n)] = ((1.0f - epsilon) * (term1 - term3) + epsilon * (term2 - term4)) * view_spacing_correction / (2.0f * epsilon * R * T_phi); // ? 1.0f / T_phi
}

__global__ void deriv_helical_NHDLH_flat(TEX_DATA g, float* __restrict__ Dg, const int4 N, const float4 T, const float4 startVal, const float R, const float D, const float tau, const float helicalPitch, const float epsilon, const float* __restrict__ phis, const int iphi_offset, const float T_ave)
{
    const int l = threadIdx.x + blockIdx.x * blockDim.x + iphi_offset;
    const int m = threadIdx.y + blockIdx.y * blockDim.y;
    const int n = threadIdx.z + blockIdx.z * blockDim.z;
...
__global__ void splitLeftAndRight(const float* g, float* g_left, float* g_right, int4 N, float4 T, float4 startVal, const float D, const float R, const float tau)
{
    const int i = threadIdx.x + blockIdx.x * blockDim.x;
    const int j = threadIdx.y + blockIdx.y * blockDim.y;
    const int k = threadIdx.z + blockIdx.z * blockDim.z;
    ...
    const float s_unbounded_threshold = tau * D / R;
    ...
    {
        s = s / D;
        const float strangeFactor = 2.0f*tau*R/(R*R - tau*tau);
        s_conj = (-s+strangeFactor)/(1.0f + s*strangeFactor);
        s_conj *= D;
        s *= D;
    }
    ...
    if (s > s_unbounded_threshold)
    {
        g_right[ind] = val;
        g_left[ind] = val_conj;
    }
    else if (s < s_unbounded_threshold)
    {
...
__global__ void mergeLeftAndRight(float* g, const float* g_left, const float* g_right, int4 N, float4 T, float4 startVal, const float D, const float R, const float tau)
{
    const int i = threadIdx.x + blockIdx.x * blockDim.x;
    const int j = threadIdx.y + blockIdx.y * blockDim.y;
    const int k = threadIdx.z + blockIdx.z * blockDim.z;
    ...
    float s_unbounded_threshold = 0.0f;
    if (tau != 0.0f)
        s_unbounded_threshold = tau * D / R;
...
__global__ void setPaddedDataKernel(float* __restrict__ data_padded, const float* __restrict__ data, int3 N, int N_pad, int startView, int endView, int numExtrapolate, const float4 T, const float4 startVals, const float R, const float helicalPitch, const float* __restrict__ weights)
{
    ...
    const float* data_block = &data[uint64(i) * uint64(N.z * N.y) + uint64(j * N.z)];

    if (helicalPitch == 0.0f || weights != NULL)
    {
        if (weights == NULL)
        {
            for (int k = 0; k < N.z; k++)
        ...
        {
            for (int k = 0; k < N.z; k++)
                data_padded_block[k] = data_block[k] * weights[k];
        }
    }
    else
    {
        ...
        const float* aProj = &data[uint64(i) * uint64(N.z * N.y)];
        for (int k = 0; k < N.z; k++)
        {
...
__global__ void setFilteredDataKernel(const float* __restrict__ data_padded, float* __restrict__ data, int3 N, int N_pad, int startView, int endView, const float4 T, const float4 startVals, const float R, const float helicalPitch, const float* __restrict__ weights)
{
    const int i = threadIdx.x + blockIdx.x * blockDim.x + startView;
    const int j = threadIdx.y + blockIdx.y * blockDim.y;
    //int j = threadIdx.x;
    ...
    if (i > endView || j > N.y - 1)
        return;
    const float* data_padded_block = &data_padded[uint64(i - startView) * uint64(N_pad * N.y) + uint64(j * N_pad)];
    ...
    if (helicalPitch == 0.0f || weights != NULL)
    {
        ...
        {
            for (int k = 0; k < N.z; k++)
                data_block[k] = data_padded_block[k] * weights[k];
        }
    }
    else
    {
        ...
        const float last_v_sample = float(N.y - 1);
        const float* aProj = &data_padded[uint64(i - startView) * uint64(N_pad * N.y)];
        for (int k = 0; k < N.z; k++)
        {
            const float theShift = helicalPitch / R * (k * T.z + startVals.z);
            const float v_new = j * T.y + startVals.y;
...
cufftComplex* HilbertTransformFrequencyResponse(int N, parameters* params, float scalar, float sampleShift)
{
    ...
    // FFT
    cufftResult result = cufftExecR2C(forward_plan, (cufftReal*)dev_h, dev_H);
...
bool finiteInverseHilbert(float*& f, parameters* params, bool data_on_cpu, float scalar, float sampleShift)
{
    if (f == NULL || params == NULL)
        return false;
    if (params->DBPparameter < 0.0)
        return false;
    LOG(logDEBUG, "ramp_filter", "finiteInverseHilbert") << "GPU " << params->whichGPU << ": start" << std::endl;

    float epsilon = params->DBPparameter;
    ...
    // PUT CODE HERE
    int N_H = optimalFFTsize(2 * params->numX);
    int N_H_over2 = N_H / 2 + 1;
    float* H_real = NULL;
    cufftComplex* H_comp = HilbertTransformFrequencyResponse(N_H, params, scalar, sampleShift);
    if (scalar != 1.0)
    {
    ...
    float helicalPitch = 0.0;

    int numAngles = params->numZ;
    int numRows = params->numY;
    //*
    if (numAngles == 1)
    {
        numRows = 1;
        numAngles = params->numY;
    }
    //*/

    int N_viewChunk = max(1, numAngles / 80); // number of views in a chunk (needs to be optimized)
    ...
    {
        fprintf(stderr, "finiteInverseHilbert: Failed to plan 1d r2c fft (size %d)\n", N_H);
        return false;
    }
    ...
    {
        cufftDestroy(forward_plan);
        fprintf(stderr, "finiteInverseHilbert: Failed to plan 1d c2r ifft\n");
        return false;
    }
    //return true;

    float* dev_f_pad = 0;
    if ((cudaStatus = cudaMalloc((void**)&dev_f_pad, uint64(N_viewChunk) * uint64(numRows) * uint64(N_H) * sizeof(float))) != cudaSuccess)
    {
        fprintf(stderr, "finiteInverseHilbert: cudaMalloc(padded volume data) failed!\n");
        printf("cudaMalloc Error: %s\n", cudaGetErrorString(cudaStatus));
        retVal = false;
    }
    ...
    if ((cudaStatus = cudaMalloc((void**)&dev_F, uint64(N_viewChunk) * uint64(numRows) * uint64(N_H_over2) * sizeof(cufftComplex))) != cudaSuccess)
    {
        fprintf(stderr, "finiteInverseHilbert: cudaMalloc(Fourier transform of padded volume data) failed!\n");
        printf("cudaMalloc Error: %s\n", cudaGetErrorString(cudaStatus));
        retVal = false;
    }
    ...
    if (H_real != NULL)
    {
        if (cudaSuccess != cudaMalloc((void**)&dev_H, N_H_over2 * sizeof(float)))
            fprintf(stderr, "finiteInverseHilbert: cudaMalloc(Fourier ramp filter) failed!\n");
        cudaStatus = cudaMemcpy(dev_H, H_real, N_H_over2 * sizeof(float), cudaMemcpyHostToDevice);
        if (cudaSuccess != cudaStatus)
        {
            fprintf(stderr, "finiteInverseHilbert: cudaMemcpy(H) failed!\n");
    ...
    else if (H_comp != NULL)
    {
        if (cudaSuccess != cudaMalloc((void**)&dev_cH, N_H_over2 * sizeof(cufftComplex)))
            fprintf(stderr, "finiteInverseHilbert: cudaMalloc(Fourier Hilbert filter) failed!\n");
    ...
    int3 origSize; origSize.x = numAngles; origSize.y = numRows; origSize.z = params->numX;

    int numExtrapolate = 0;
    //if (params->truncatedScan)
    //    numExtrapolate = min(N_H - params->numX - 1, 100);

    if (retVal == true)
    {
        float* w = new float[params->numX];
        float* w_inv = new float[params->numX];

        float L_1 = 0;
        float U_1 = float(params->numX-1);

        float L = epsilon;
        float U = float(params->numX-1) - epsilon;
        for (int i = 0; i < params->numX; i++)
        {
            float arg_1 = (float(i) - L_1) * (U_1 - float(i));
            float k_1 = 0.0;
            if (arg_1 > 0.0)
                k_1 = sqrt(arg_1);

            float arg = (float(i) - L) * (U - float(i));
            float k = 0.0;
            if (arg > 0.0)
                k = sqrt(arg);
        ...
        float* dev_w = copy1DdataToGPU(w, params->numX, params->whichGPU);
        float* dev_w_inv = copy1DdataToGPU(w_inv, params->numX, params->whichGPU);
        //float* dev_w = NULL;
        //float* dev_w_inv = NULL;
        delete [] w;
        delete [] w_inv;
        ...
        for (int iChunk = 0; iChunk < numChunks; iChunk++)
        {
            ...
            setPaddedDataKernel <<< dimGrid_setting, dimBlock_setting >>> (dev_f_pad, dev_f, origSize, N_H, startView, endView, numExtrapolate, T_f, startVal_f, params->sod, helicalPitch, dev_w);

            // FFT
            cufftResult result = cufftExecR2C(forward_plan, (cufftReal*)dev_f_pad, dev_F);
            ...
            // Multiply Filter
            if (dev_H != 0)
                multiplyRampFilterKernel <<< dimGrid_viewChunk, dimBlock_viewChunk >>> (dev_F, dev_H, dataSize);
            else if (dev_cH != 0)
                multiplyComplexFilterKernel <<< dimGrid_viewChunk, dimBlock_viewChunk >>> (dev_F, dev_cH, dataSize);
            ...
            setFilteredDataKernel <<< dimGrid_setting, dimBlock_setting >>> (dev_f_pad, dev_f, origSize, N_H, startView, endView, T_f, startVal_f, params->sod, helicalPitch, dev_w_inv);
        }
        ...
        if (data_on_cpu)
        {
            pullVolumeDataFromGPU(f, params, dev_f, params->whichGPU, true);
        }
    }
    ...
    if (retVal)
    {
        LOG(logDEBUG, "ramp_filter", "finiteInverseHilbert") << "GPU " << params->whichGPU << ": completed successfully" << std::endl;
    }
    else
    {
        LOG(logDEBUG, "ramp_filter", "finiteInverseHilbert") << "GPU " << params->whichGPU << ": completed with errors" << std::endl;
    }
...
bool conv1D(float*& g, parameters* params, bool data_on_cpu, float scalar, int which, float sampleShift)
{
    ...
    if (H_comp == NULL || params->numRows == 1)
        helicalPitch = 0.0;
    helicalPitch = 0.0;
    //helicalPitch = params->helicalPitch;
    ...
    if (CUFFT_SUCCESS != cufftPlan1d(&forward_plan, N_H, CUFFT_R2C, uint64(N_viewChunk) * uint64(numRows)))
    {
        fprintf(stderr, "conv1D: Failed to plan 1d r2c fft (size %d)\n", N_H);
        return false;
    }
    cufftHandle backward_plan;
    if (CUFFT_SUCCESS != cufftPlan1d(&backward_plan, N_H, CUFFT_C2R, uint64(N_viewChunk) * uint64(numRows))) // do I use N_H_over2?
    {
        fprintf(stderr, "conv1D: Failed to plan 1d c2r ifft\n");
        return false;
    }
    //return true;
    ...
    {
        fprintf(stderr, "conv1D: cudaMalloc(padded projection data) failed!\n");
        printf("cudaMalloc Error: %s\n", cudaGetErrorString(cudaStatus));
        retVal = false;
    }
    ...
    {
        fprintf(stderr, "conv1D: cudaMalloc(Fourier transform of padded projection data) failed!\n");
        printf("cudaMalloc Error: %s\n", cudaGetErrorString(cudaStatus));
        retVal = false;
    }
    ...
    {
        if (cudaSuccess != cudaMalloc((void**)&dev_H, N_H_over2 * sizeof(float)))
            fprintf(stderr, "conv1D: cudaMalloc(Fourier ramp filter) failed!\n");
        cudaStatus = cudaMemcpy(dev_H, H_real, N_H_over2 * sizeof(float), cudaMemcpyHostToDevice);
        if (cudaSuccess != cudaStatus)
        {
            fprintf(stderr, "conv1D: cudaMemcpy(H) failed!\n");
            fprintf(stderr, "error name: %s\n", cudaGetErrorName(cudaStatus));
            fprintf(stderr, "error msg: %s\n", cudaGetErrorString(cudaStatus));
            retVal = false;
        }
    ...
    {
        if (cudaSuccess != cudaMalloc((void**)&dev_cH, N_H_over2 * sizeof(cufftComplex)))
            fprintf(stderr, "conv1D: cudaMalloc(Fourier Hilbert filter) failed!\n");
        cudaStatus = cudaMemcpy(dev_cH, H_comp, N_H_over2 * sizeof(cufftComplex), cudaMemcpyHostToDevice);
        if (cudaSuccess != cudaStatus)
        {
            fprintf(stderr, "conv1D: cudaMemcpy(H) failed!\n");
    ...
    if (retVal == true)
    {
        ...
        for (int iChunk = 0; iChunk < numChunks; iChunk++)
        {
            ...
            setPaddedDataKernel <<< dimGrid_setting, dimBlock_setting >>> (dev_g_pad, dev_g, origSize, N_H, startView, endView, numExtrapolate, T_g, startVal_g, params->sod, helicalPitch, NULL);
            //cudaDeviceSynchronize();
            ...
            // FFT
            cufftResult result = cufftExecR2C(forward_plan, (cufftReal*)dev_g_pad, dev_G);
            if (result != CUFFT_SUCCESS)
            {
                printf("cufftExecR2C failed!\n");
            }
            ...
            setFilteredDataKernel <<< dimGrid_setting, dimBlock_setting >>> (dev_g_pad, dev_g, origSize, N_H, startView, endView, T_g, startVal_g, params->sod, helicalPitch, NULL);
            //cudaDeviceSynchronize();
        }
        cudaDeviceSynchronize();
...
bool transmissionFilter_gpu(float*& g, parameters* params, bool data_on_cpu, float* H_full, int N_H1, int N_H2, bool isAttenuationData, float FWHM)
{
    ...
    /*
    Note that setPaddedDataFor2DFilter_reverse does this:
    t = t + h*t <==> T = (1+H)T
    That is why we subtract one from the frequency response below
    //*/
    ...
    for (int j = 0; j < N_H1; j++)
    {
        float y;
        if (j < N_H1 / 2)
            y = float(j)*2.0/float(N_H1);
        else
            y = (double(j) - double(N_H1)) * 2.0 / float(N_H1);
        for (int i = 0; i < N_H2_over2; i++)
        {
            float x;
            if (i < N_H2 / 2)
                x = float(i)*2.0/float(N_H2);
            else
                x = (double(i) - double(N_H2)) * 2.0 / float(N_H2);
            float r = sqrt(x*x + y*y);

            if (FWHM <= 1.0)
                H[j * N_H2_over2 + i] = H_full[j * N_H2 + i] - 1.0;
            else
                H[j * N_H2_over2 + i] = (H_full[j * N_H2 + i] - 1.0) * smooth_rect(r, 1.0/FWHM);
        }
    }
    ...
    for (int k = 0; k < N_z; k++)
    {
        ...
        if (retVal)
        {
            ...
            dimGrid_padding.y = int(ceil(double(params->numCols) / double(dimBlock_padding.y)));
...
bool rampFilter2D(float*& f, parameters* params, bool data_on_cpu)
{
    ...
    for (int k = 0; k < N_z; k++)
    {
        ...
        if (retVal)
        {
            ...
            dimGrid_padding.y = int(ceil(double(params->numX) / double(dimBlock_padding.y)));
...
bool Laplacian_gpu(float*& g, int numDims, bool smooth, parameters* params, bool data_on_cpu, float scalar)
{
    ...
    if (data_on_cpu)
    {
        ...
        if (cudaSuccess != cudaMalloc((void**)&dev_Dg, params->projectionData_numberOfElements() * sizeof(float)))
            fprintf(stderr, "Laplacian_gpu: cudaMalloc failed!\n");
    }
    else
    {
        dev_Dg = g;
    ...
    {
        fprintf(stderr, "Laplacian_gpu: kernel failed!\n");
        fprintf(stderr, "error name: %s\n", cudaGetErrorName(cudaStatus));
        fprintf(stderr, "error msg: %s\n", cudaGetErrorString(cudaStatus));
    }
...
bool ray_derivative(float*& g, parameters* params, bool data_on_cpu, float scalar, float sampleShift)
{
    ...
    {
        printf("ray_derivative: invalid inputs\n");
        return false;
    }
    //fprintf(stderr, "ray_derivative: using projection[%d][%d][%d] data\n", params->numAngles, params->numRows, params->numCols);
    cudaSetDevice(params->whichGPU);
    cudaError_t cudaStatus;
    ...
    float* dev_h = 0;
    int N_h = 0;
    if (params->doDBP == true)
    {
        float* h = first_order_finite_difference_filter(N_h, params->rampID, int(sampleShift));
        if (N_h % 2 == 0)
        {
            float* h_pad = (float*) malloc(sizeof(float)*size_t(N_h+1));
            if (sampleShift > 0.0)
            {
                for (int i = 0; i < N_h; i++)
                    h_pad[i+1] = h[i];
        ...
        for (int i = 0; i < N_h; i++)
            h[i] *= scalar / T_g.z;
        dev_h = copy1DdataToGPU(h, N_h, params->whichGPU);
        free(h);
    ...
    if (dev_g == 0)
    {
        fprintf(stderr, "ray_derivative: failed to copy projection[%d][%d][%d] data!\n", params->numAngles, params->numRows, params->numCols);
        return false;
    }

    dim3 dimBlock(8, 8);
    dim3 dimGrid(int(ceil(double(params->numAngles) / double(dimBlock.x))), int(ceil(double(params->numRows) / double(dimBlock.y))));
    if (dev_h == 0)
        ray_derivative_kernel_inplace <<< dimGrid, dimBlock >>> (dev_g, N_g, T_g, startVal_g, scalar, sampleShift);
    else
        ray_derivative_kernel_inplace_high_order <<< dimGrid, dimBlock >>> (dev_g, N_g, dev_h, N_h);
    params->colShiftFromFilter += float(-0.5*sampleShift); // opposite sign as LTT

    cudaStatus = cudaDeviceSynchronize();
    if (cudaStatus != cudaSuccess)
    {
        fprintf(stderr, "ray_derivative: kernel failed!\n");
        fprintf(stderr, "error name: %s\n", cudaGetErrorName(cudaStatus));
        fprintf(stderr, "error msg: %s\n", cudaGetErrorString(cudaStatus));
        fprintf(stderr, "ray_derivative: using projection[%d][%d][%d] data!\n", params->numAngles, params->numRows, params->numCols);
    }
    ...
    if (data_on_cpu)
    {
        dev_g = copyProjectionDataToGPU(g, params, params->whichGPU);
        if (cudaSuccess != cudaMalloc((void**)&dev_Dg, params->projectionData_numberOfElements() * sizeof(float)))
            fprintf(stderr, "ray_derivative: cudaMalloc failed!\n");
    }
    else
    {
        dev_Dg = g;
...
bool parallelRay_derivative(float*& g, parameters* params, bool data_on_cpu)
{
    ...
    if (data_on_cpu)
    {
        dev_g = copyProjectionDataToGPU(g, params, params->whichGPU);
        if (cudaSuccess != cudaMalloc((void**)&dev_Dg, params->projectionData_numberOfElements() * sizeof(float)))
            fprintf(stderr, "parallelRay_derivative: cudaMalloc failed!\n");
    }
    else
    {
        dev_Dg = g;
    ...
    bool detector_rotated = setConstantMemoryGeometryParameters_filtering(params);

    dim3 dimBlock = setBlockSize(N_g);
    dim3 dimGrid = setGridSize(N_g, dimBlock);
    if (params->detectorType == parameters::FLAT)
    {
        float T_ave = params->T_phi();
        if (detector_rotated)
            deriv_helical_NHDLH_flat_rot <<< dimGrid, dimBlock >>> (d_data_txt, dev_Dg, N_g, T_g, startVal_g, params->sod, params->sdd, params->tau, params->helicalPitch, epsilon, dev_phis, 0, T_ave);
        else
            deriv_helical_NHDLH_flat <<< dimGrid, dimBlock >>> (d_data_txt, dev_Dg, N_g, T_g, startVal_g, params->sod, params->sdd, params->tau, params->helicalPitch, epsilon, dev_phis, 0, T_ave);
    }
    else
    ...
    if (cudaStatus != cudaSuccess)
    {
        fprintf(stderr, "parallelRay_derivative: kernel failed!\n");
        fprintf(stderr, "error name: %s\n", cudaGetErrorName(cudaStatus));
        fprintf(stderr, "error msg: %s\n", cudaGetErrorString(cudaStatus));
    }
...
bool parallelRay_derivative_chunk(float*& g, parameters* params, bool data_on_cpu)
{
    ...
    //float epsilon = float(std::min(0.01, T_g.z / (4.0 * fabs(params->T_phi()))));
    float epsilon = float(std::min(0.5, T_g.z / (4.0 * fabs(params->T_phi()))));
    if (params->helicalFilterParameter > 0.0)
        epsilon = float(std::min(float(0.5), params->helicalFilterParameter));
    //float epsilon = T_g.z / (4.0 * fabs(params->T_phi()));
    //printf("elsilon = %f\n", epsilon);
    //epsilon = 0.026044*0.5;
    //epsilon = 0.01;

    // floor of 2: smaller values launch zero-view equal() copies below (numAngles 2-3),
    // which poison the CUDA error state, or divide by zero computing numChunks (numAngles 1)
    int maxChunkSize = max(2, min(params->numAngles/2, 100));
    ...
    if (cudaSuccess != cudaMalloc((void**)&dev_Dg, N_g_chunk_prod * sizeof(float)))
        fprintf(stderr, "parallelRay_derivative_chunk: cudaMalloc failed!\n");
    ...
    for (int ichunk = 0; ichunk < numChunks; ichunk++)
    {
        ...
        {
            if (detector_rotated)
                deriv_helical_NHDLH_flat_rot <<< dimGrid, dimBlock >>> (d_data_txt, dev_Dg, N_g_chunk, T_g, startVal_g, params->sod, params->sdd, params->tau, params->helicalPitch, epsilon, dev_phis_chunk, 0, T_ave);
            else
                deriv_helical_NHDLH_flat <<< dimGrid, dimBlock >>> (d_data_txt, dev_Dg, N_g_chunk, T_g, startVal_g, params->sod, params->sdd, params->tau, params->helicalPitch, epsilon, dev_phis_chunk, 0, T_ave);
        }
        else
        ...
        // the derivative of this chunk is now saved in dev_Dg (with padding)
        ...
        {
            // g[this chunk] = Dg[all but padded views]
        ...
        {
            // g = Dg, but don't copy last view
            // this last view if need for the derivative calculation in the next chunk
            // but we still need this derivative so we save it in dev_interface
            ...
            float* dev_Dg_last = &dev_Dg[uint64(iphi_hi - iphi_pad_lo) * uint64(N_g_chunk.y * N_g_chunk.z)];
    ...
    if (cudaStatus != cudaSuccess)
    {
        fprintf(stderr, "parallelRay_derivative_chunk: kernel failed!\n");
        fprintf(stderr, "error name: %s\n", cudaGetErrorName(cudaStatus));
        fprintf(stderr, "error msg: %s\n", cudaGetErrorString(cudaStatus));
    }
...
bool rampFilter1D_symmetric(float*& g, parameters* params, float scalar)
{
    ...
    splitLeftAndRight <<< dimGrid, dimBlock >>> (dev_g, dev_g_left, dev_g_right, N_g, T_g, startVal_g, params->sdd, params->sod, params->tau);
    cudaStatus = cudaDeviceSynchronize();
    if (cudaStatus != cudaSuccess)
    {
        fprintf(stderr, "rampFilter1D_symmetric: kernel failed!\n");
        fprintf(stderr, "error message: %s\n", cudaGetErrorString(cudaStatus));
    }
    ...
    // Merge back to g
    mergeLeftAndRight <<< dimGrid, dimBlock >>> (dev_g, dev_g_left, dev_g_right, N_g, T_g, startVal_g, params->sdd, params->sod, params->tau);
    cudaStatus = cudaDeviceSynchronize();
    if (cudaStatus != cudaSuccess)
    {
        fprintf(stderr, "rampFilter1D_symmetric: kernel failed!\n");
...
float* zeroPadForOffsetScan_GPU(float* g, parameters* params, float* g_out, bool data_on_cpu)
{
    ...
    if (N_add > 0 && offsetScanWeights != NULL)
    {
        ...
        if (g_out == NULL)
        {
            ...
            {
                fprintf(stderr, "zeroPadForOffsetScan_GPU: cudaMalloc(projections) failed!\n");
                fprintf(stderr, "error message: %s\n", cudaGetErrorString(cudaStatus));
                return NULL;
            }
        }
        ...
        if (cudaStatus != cudaSuccess)
        {
            fprintf(stderr, "zeroPadForOffsetScan_GPU: kernel failed!\n");
            fprintf(stderr, "error message: %s\n", cudaGetErrorString(cudaStatus));
        }
...
bool conv1D(float*& g, parameters* params, bool data_on_cpu, float scalar, int which, float sampleShift)
{
    ...
    TEX_DATA d_h_txt = {};
    TEX_ARRAY d_h_array = loadTexture1D(d_h_txt, h, N_H, false, false);
...
bool setConstantMemoryGeometryParameters_filtering(parameters* params)
{
    return false;
