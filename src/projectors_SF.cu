...
__global__ void coneParallelWeightedHelicalBackprojectorKernel_SF(TEX_DATA g, int4 N_g, float4 T_g, float4 startVals_g, float* f, int4 N_f, float4 T_f, float4 startVals_f, const float R, const float D, const float tau, const float rFOVsq, const float* phis, const int volumeDimensionOrder, bool accum)
{
    ...
    for (int l = 0; l < N_g.x; l++)
    {
        ...
        if (centralWeight > 0.0f)
        {
            ...
            if (z_high > 1.0f)
            {
                float val1 = TEX3D(g, N_g, s_ind_A, row_high + v_oneAndTwo, L);
                float val2 = TEX3D(g, N_g, s_ind_A, row_high_plus_two, L);
                val += (val1 * (v_weight_one + v_weight_two) + val2 * (z_high - 1.0f)) * bpWeight;
            ...
            {
                float val1 = TEX3D(g, N_g, s_ind_A, row_high + v_oneAndTwo, L);
                val += val1 * (v_weight_one + v_weight_two) * bpWeight;
    ...
    for (int l = 0; l < N_g.x; l++)
    {
        ...
        for (int k_offset = 0; k_offset < numZ; k_offset++)
        {
            ...
            if (z_high > 1.0f)
            {
                float val1 = TEX3D(g, N_g, s_ind_A, row_high + v_oneAndTwo, L);
                float val2 = TEX3D(g, N_g, s_ind_A, row_high_plus_two, L);
                vals[k_offset] += (val1 * (v_weight_one + v_weight_two) + val2 * (z_high - 1.0f)) * bpWeight;
            ...
            {
                float val1 = TEX3D(g, N_g, s_ind_A, row_high + v_oneAndTwo, L);
                vals[k_offset] += val1 * (v_weight_one + v_weight_two) * bpWeight;
...
__global__ void coneParallelProjectorKernel_SF(float* g, int4 N_g, float4 T_g, float4 startVals_g, TEX_DATA f, int4 N_f, float4 T_f, float4 startVals_f, float R, float D, float tau, float rFOVsq, float* phis, int volumeDimensionOrder, bool accum)
{
    ...
    if (fabs(cos_phi) > fabs(sin_phi))
    {
        ...
        for (int i = 0; i < N_f.x; i++)
        {
            ...
            if (volumeDimensionOrder == 0)
            {
                float val1 = TEX3D(f, N_f, z_12, y_12, float(i) + 0.5f);
                float val2 = TEX3D(f, N_f, float(k + 2) + 0.5f, y_12, float(i) + 0.5f);
                float val3 = TEX3D(f, N_f, z_12, float(j_min_A + 2) + 0.5f, float(i) + 0.5f);
                float val4 = TEX3D(f, N_f, float(k + 2) + 0.5f, float(j_min_A + 2) + 0.5f, float(i) + 0.5f);
                g_output += (val1 * (vWeight_0 + vWeight_1) + val2 * vWeight_2) * (hWeight_0 + hWeight_1)
                         + (val3 * (vWeight_0 + vWeight_1) + val4 * vWeight_2) * hWeight_2;
            ...
            {
                float val1 = TEX3D(f, N_f, float(i) + 0.5f, y_12, z_12);
                float val2 = TEX3D(f, N_f, float(i) + 0.5f, y_12, float(k + 2) + 0.5f);
                float val3 = TEX3D(f, N_f, float(i) + 0.5f, float(j_min_A + 2) + 0.5f, z_12);
                float val4 = TEX3D(f, N_f, float(i) + 0.5f, float(j_min_A + 2) + 0.5f, float(k + 2) + 0.5f);
                g_output += (val1 * (vWeight_0 + vWeight_1) + val2 * vWeight_2) * (hWeight_0 + hWeight_1)
                         + (val3 * (vWeight_0 + vWeight_1) + val4 * vWeight_2) * hWeight_2;
    ...
    else
    {
        ...
        for (int j = 0; j < N_f.y; j++)
        {
            ...
            if (volumeDimensionOrder == 0)
            {
                float val1 = TEX3D(f, N_f, z_12, float(j) + 0.5f, x_12);
                float val2 = TEX3D(f, N_f, float(k + 2) + 0.5f, float(j) + 0.5f, x_12);
                float val3 = TEX3D(f, N_f, z_12, float(j) + 0.5f, float(i_min_A + 2) + 0.5f);
                float val4 = TEX3D(f, N_f, float(k + 2) + 0.5f, float(j) + 0.5f, float(i_min_A + 2) + 0.5f);
                g_output += (val1 * (vWeight_0 + vWeight_1) + val2 * vWeight_2) * (hWeight_0 + hWeight_1)
                         + (val3 * (vWeight_0 + vWeight_1) + val4 * vWeight_2) * hWeight_2;
            ...
            {
                float val1 = TEX3D(f, N_f, x_12, float(j) + 0.5f, z_12);
                float val2 = TEX3D(f, N_f, x_12, float(j) + 0.5f, float(k + 2) + 0.5f);
                float val3 = TEX3D(f, N_f, float(i_min_A + 2) + 0.5f, float(j) + 0.5f, z_12);
                float val4 = TEX3D(f, N_f, float(i_min_A + 2) + 0.5f, float(j) + 0.5f, float(k + 2) + 0.5f);
                g_output += (val1 * (vWeight_0 + vWeight_1) + val2 * vWeight_2) * (hWeight_0 + hWeight_1)
                         + (val3 * (vWeight_0 + vWeight_1) + val4 * vWeight_2) * hWeight_2;
...
__global__ void parallelBeamBackprojectorKernel_SF(TEX_DATA g, int4 N_g, float4 T_g, float4 startVals_g, float* f, int4 N_f, float4 T_f, float4 startVals_f, float rFOVsq, float* phis, int volumeDimensionOrder, bool accum)
{
    ...
    for (int l = 0; l < N_g.x; l++)
    {
        ...
        for (int k_offset = 0; k_offset < numZ; k_offset++) {
            vals[k_offset] += TEX3D_linear(g, N_g, s_ind_A, float(k+k_offset) + 0.5f, float(l) + 0.5f);
...
__global__ void parallelBeamProjectorKernel_SF(float* g, int4 N_g, float4 T_g, float4 startVals_g, TEX_DATA f, int4 N_f, float4 T_f, float4 startVals_f, float rFOVsq, float* phis, int volumeDimensionOrder, bool accum)
{
    const int l = threadIdx.x + blockIdx.x * blockDim.x;
    const int m = threadIdx.y + blockIdx.y * blockDim.y;
    const int n = threadIdx.z + blockIdx.z * blockDim.z;
    ...
    if (fabs(cos_phi) > fabs(sin_phi))
    {
        ...
        for (int i = 0; i < N_f.x; i++)
        {
            ...
            if (volumeDimensionOrder == 0)
            {
                float val1 = TEX3D_linear(f, N_f, float(m) + 0.5f, float(j_min_A) + 0.5f +  weight_1/(weight_0+weight_1), float(i) + 0.5f);
                float val2 = TEX3D_linear(f, N_f, float(m) + 0.5f, float(j_min_A + 2) + 0.5f, float(i) + 0.5f);
                g_output += (weight_0 + weight_1) * val1 
                         + max(0.0f, min(n_plus_half, s_ind_A + 2.0f * ds_ind_dj + C) 
                         - max(n_minus_half, s_ind_A + 2.0f * ds_ind_dj - C)) * val2;
            ...
            {
                float val1 = TEX3D_linear(f, N_f, float(i) + 0.5f, float(j_min_A) + 0.5f + weight_1/(weight_0+weight_1), float(m) + 0.5f);
                float val2 = TEX3D_linear(f, N_f, float(i) + 0.5f, float(j_min_A + 2) + 0.5f, float(m)+0.5f);
                g_output += (weight_0 + weight_1) * val1
                         + max(0.0f, min(n_plus_half, s_ind_A + 2.0f * ds_ind_dj + C) 
                         - max(n_minus_half, s_ind_A + 2.0f * ds_ind_dj - C)) * val2;
    ...
    else
    {
        ...
        for (int j = 0; j < N_f.y; j++)
        {
            ...
            if (volumeDimensionOrder == 0)
            {
                float val1 = TEX3D_linear(f, N_f, float(m) + 0.5f, float(j) + 0.5f, float(i_min_A)+0.5f + weight_1/(weight_0 + weight_1));
                float val2 = TEX3D_linear(f, N_f, float(m) + 0.5f, float(j) + 0.5f, float(i_min_A + 2)+0.5f);
                g_output += (weight_0 + weight_1) * val1 
                         + max(0.0f, min(n_plus_half, s_ind_A + 2.0f * ds_ind_di + C) 
                         - max(n_minus_half, s_ind_A + 2.0f * ds_ind_di - C)) * val2;
            ...
            {
                float val1 = TEX3D_linear(f, N_f, float(i_min_A) + 0.5f + weight_1 / (weight_0 + weight_1), float(j) + 0.5f, float(m) + 0.5f);
                float val2 = TEX3D_linear(f, N_f, float(i_min_A + 2) + 0.5f, float(j) + 0.5f, float(m) + 0.5f);
                g_output += (weight_0 + weight_1) * val1
                         + max(0.0f, min(n_plus_half, s_ind_A + 2.0f * ds_ind_di + C)
                         - max(n_minus_half, s_ind_A + 2.0f * ds_ind_di - C)) * val2;
...
__global__ void fanBeamBackprojectorKernel_SF(TEX_DATA g, int4 N_g, float4 T_g, float4 startVals_g, float* f, int4 N_f, float4 T_f, float4 startVals_f, float R, float D, float tau, float rFOVsq, float* phis, int volumeDimensionOrder, bool doWeight, bool accum)
{
    ...
    for (int l = 0; l < N_g.x; l++)
    {
        ...
        for (int k_offset = 0; k_offset < numZ; k_offset++)
        {
            float val1 = TEX3D(g, N_g, ind_first, iv + float(k_offset), L);
            float val2 = TEX3D(g, N_g, ind_last, iv + float(k_offset), L);
            vals[k_offset] += (val1 * horizontalWeights_0_A + val2 * horizontalWeights_1_A) * bpWeight;
...
__global__ void fanBeamProjectorKernel_SF(float* g, int4 N_g, float4 T_g, float4 startVals_g, TEX_DATA f, int4 N_f, float4 T_f, float4 startVals_f, float R, float D, float tau, float rFOVsq, float* phis, int volumeDimensionOrder, bool accum)
{
    const int l = threadIdx.x + blockIdx.x * blockDim.x;
    const int m = threadIdx.y + blockIdx.y * blockDim.y;
    const int n = threadIdx.z + blockIdx.z * blockDim.z;
    ...
    if (fabs(u * cos_phi - sin_phi) > fabs(u * sin_phi + cos_phi))
    {
        ...
        for (int j = 0; j < N_f.y; j++)
        {
            ...
            if (volumeDimensionOrder == 0)
            {
                float val1 = TEX3D(f, N_f, float(iz) + 0.5f, float(j) + 0.5f, float(i) + 0.5f + hWeight_1/(hWeight_0 + hWeight_1));
                float val2 = TEX3D(f, N_f, float(iz) + 0.5f, float(j) + 0.5f, float(i + 2)+0.5f);
                g_output += val1 * (hWeight_0 + hWeight_1) + val2 * hWeight_2;
            ...
            {
                float val1 = TEX3D(f, N_f, float(i) + 0.5f + hWeight_1 / (hWeight_0 + hWeight_1), float(j) + 0.5f, float(iz) + 0.5f);
                float val2 = TEX3D(f, N_f, float(i + 2) + 0.5f, float(j) + 0.5f, float(iz) + 0.5f);
                g_output += val1 * (hWeight_0 + hWeight_1) + val2 * hWeight_2;
    ...
    else
    {
        ...
        for (int i = 0; i < N_f.x; i++)
        {
            ...
            if (volumeDimensionOrder == 0)
            {
                float val1 = TEX3D(f, N_f, float(iz) + 0.5f, float(j) + 0.5f + hWeight_1/(hWeight_0 + hWeight_1), float(i) + 0.5f);
                float val2 = TEX3D(f, N_f, float(iz) + 0.5f, float(j + 2) + 0.5f, float(i) + 0.5f);
                g_output += val1 * (hWeight_0 + hWeight_1) + val2 * hWeight_2;
            ...
            {
                float val1 = TEX3D(f, N_f, float(i) + 0.5f, float(j) + 0.5f+hWeight_1 / (hWeight_0 + hWeight_1), float(iz) + 0.5f);
                float val2 = TEX3D(f, N_f, float(i) + 0.5f, float(j + 2) + 0.5f, float(iz)+0.5f);
                g_output += val1 * (hWeight_0 + hWeight_1) + val2 * hWeight_2;
    ...
    for (int l = 0; l < N_g.x; l++)
    {
        ...
        if (centralWeight > 0.0f)
        {
            ...
            if (z_high_A > 1.0f)
            {
                ...
                float val1 = TEX3D(g, N_g, ind_first, row_high_A + v_oneAndTwo, L);
                float val2 = TEX3D(g, N_g, ind_last, row_high_A + v_oneAndTwo, L);
                float val3 = TEX3D(g, N_g, ind_first, row_high_plus_two_A, L);
                float val4 = TEX3D(g, N_g, ind_last, row_high_plus_two_A, L);
                val += ((val1 * horizontalWeights_0_A + val2 * horizontalWeights_1_A) * (v_weight_one + v_weight_two)
                    + (val3 * horizontalWeights_0_A + val4 * horizontalWeights_1_A) * (z_high_A - 1.0f)) * extraWeight * centralWeight / (centralWeight + sumWeights);
            ...
            else
            {
                ...
                float val1 = TEX3D(g, N_g, ind_first, row_high_A + v_oneAndTwo, L);
                float val2 = TEX3D(g, N_g, ind_last, row_high_A + v_oneAndTwo, L);
                val += ((val1 * horizontalWeights_0_A + val2 * horizontalWeights_1_A) * (v_weight_one + v_weight_two)) * extraWeight * centralWeight / (centralWeight + sumWeights);
...
__global__ void coneBeamHelicalWeightedBackprojectorKernel_SF(TEX_DATA g, int4 N_g, float4 T_g, float4 startVals_g, float* f, int4 N_f, float4 T_f, float4 startVals_f, float R, float D, float tau, float rFOVsq, float* phis, int volumeDimensionOrder, bool accum)
{
    ...
    for (int l = 0; l < N_g.x; l++)
    {
        ...
        {   
            ...
            float val4 = TEX3D(g, N_g, ind_last, row_high_plus_two_A, L);
            val += ((val1 * horizontalWeights_0_A + val2 * horizontalWeights_1_A) * (v_weight_one + v_weight_two)
                  + (val3 * horizontalWeights_0_A + val4 * horizontalWeights_1_A) * max(0.0f, z_high_A - 1.0f)) * extraWeight * centralWeight / (centralWeight + sumWeights);
    ...
    for (int l = 0; l < N_g.x; l++)
    {
        ...
        for (int k_offset = 0; k_offset < numZ; k_offset++)
        {
            ...
            if (z_high_A > 1.0f)
            {
                ...
                float val3 = TEX3D(g, N_g, ind_first, row_high_plus_two_A, L);
                float val4 = TEX3D(g, N_g, ind_last, row_high_plus_two_A, L);
                vals[k_offset] += ((val1 * horizontalWeights_0_A + val2 * horizontalWeights_1_A) * (v_weight_one + v_weight_two)
                               + (val3 * horizontalWeights_0_A + val4 * horizontalWeights_1_A) * (z_high_A - 1.0f)) * v_weight;
            ...
            else
            {
                float val1 = TEX3D(g, N_g, ind_first, row_high_A + v_oneAndTwo, L);
                float val2 = TEX3D(g, N_g, ind_last, row_high_A + v_oneAndTwo, L);
                vals[k_offset] += ((val1 * horizontalWeights_0_A + val2 * horizontalWeights_1_A) * (v_weight_one + v_weight_two)) * v_weight;
...
__global__ void curvedConeBeamProjectorKernel_SF(float* g, int4 N_g, float4 T_g, float4 startVals_g, TEX_DATA f, int4 N_f, float4 T_f, float4 startVals_f, float R, float D, float tau, float rFOVsq, float* phis, int volumeDimensionOrder, bool accum)
{
    const int l = threadIdx.x + blockIdx.x * blockDim.x;
    const int m = threadIdx.y + blockIdx.y * blockDim.y;
    const int n = threadIdx.z + blockIdx.z * blockDim.z;
    ...
    if (fabs(u * cos_phi - sin_phi) > fabs(u * sin_phi + cos_phi))
    {
        ...
        for (int j = 0; j < N_f.y; j++)
        {
            ...
            if (volumeDimensionOrder == 0)
            {
                float val1 = TEX3D(f, N_f, z_12, float(j) + 0.5f, x_12);
                float val2 = TEX3D(f, N_f, float(k + 2) + 0.5f, float(j) + 0.5f, x_12);
                float val3 = TEX3D(f, N_f, z_12, float(j) + 0.5f, float(i + 2) + 0.5f);
                float val4 = TEX3D(f, N_f, float(k + 2) + 0.5f, float(j) + 0.5f, float(i + 2) + 0.5f);
                g_output += (val1 * (vWeight_0 + vWeight_1) + val2 * vWeight_2) * (hWeight_0 + hWeight_1)
                         + (val3 * (vWeight_0 + vWeight_1) + val4 * vWeight_2) * hWeight_2;
            ...
            else
            {
                float val1 = TEX3D(f, N_f, x_12, float(j) + 0.5f, z_12);
                float val2 = TEX3D(f, N_f, x_12, float(j) + 0.5f, float(k + 2) + 0.5f);
                float val3 = TEX3D(f, N_f, float(i + 2) + 0.5f, float(j) + 0.5f, z_12);
                float val4 = TEX3D(f, N_f, float(i + 2) + 0.5f, float(j) + 0.5f, float(k + 2) + 0.5f);
                g_output += (val1 * (vWeight_0 + vWeight_1) + val2 * vWeight_2) * (hWeight_0 + hWeight_1)
                         + (val3 * (vWeight_0 + vWeight_1) + val4 * vWeight_2) * hWeight_2;
    ...
    else
    {
        ...
        for (int i = 0; i < N_f.x; i++)
        {
            ...
            if (volumeDimensionOrder == 0)
            {
                float val1 = TEX3D(f, N_f, z_12, y_12, float(i) + 0.5f);
                float val2 = TEX3D(f, N_f, float(k + 2) + 0.5f, y_12, float(i) + 0.5f);
                float val3 = TEX3D(f, N_f, z_12, float(j + 2) + 0.5f, float(i) + 0.5f);
                float val4 = TEX3D(f, N_f, float(k + 2) + 0.5f, float(j + 2) + 0.5f, float(i) + 0.5f);
                g_output += (val1 * (vWeight_0 + vWeight_1) + val2 * vWeight_2) * (hWeight_0 + hWeight_1)
                         + (val3 * (vWeight_0 + vWeight_1) + val4 * vWeight_2) * hWeight_2;
            ...
            else
            {
                float val1 = TEX3D(f, N_f, float(i) + 0.5f, y_12, z_12);
                float val2 = TEX3D(f, N_f, float(i) + 0.5f, y_12, float(k + 2) + 0.5f);
                float val3 = TEX3D(f, N_f, float(i) + 0.5f, float(j + 2) + 0.5f, z_12);
                float val4 = TEX3D(f, N_f, float(i) + 0.5f, float(j + 2) + 0.5f, float(k + 2) + 0.5f);
                g_output += (val1 * (vWeight_0 + vWeight_1) + val2 * vWeight_2) * (hWeight_0 + hWeight_1)
                         + (val3 * (vWeight_0 + vWeight_1) + val4 * vWeight_2) * hWeight_2;
    ...
    if (tiltAngle == 0.0f)
    {
        ...
        for (int l = 0; l < N_g.x; l++)
        {
            ...
            for (int k_offset = 0; k_offset < numZ; k_offset++)
            {
                ...
                if (z_high_A > 1.0f)
                {
                    float val1 = TEX3D_linear(g, N_g, ind_first, row_high_A + v_oneAndTwo, L);
                    float val2 = TEX3D_linear(g, N_g, ind_last, row_high_A + v_oneAndTwo, L);
                    float val3 = TEX3D_linear(g, N_g, ind_first, row_high_plus_two_A, L);
                    float val4 = TEX3D_linear(g, N_g, ind_last, row_high_plus_two_A, L);
                    vals[k_offset] += ((val1 * horizontalWeights_0_A + val2 * horizontalWeights_1_A) * (v_weight_one + v_weight_two)
                                   + (val3 * horizontalWeights_0_A + val4 * horizontalWeights_1_A) * (z_high_A - 1.0f)) * v_weight;
    ...
    else
    {
        ...
        for (int l = 0; l < N_g.x; l++)
        {
            ...
            for (int k_offset = 0; k_offset < numZ; k_offset++)
            {
                ...
                if (vWeights_1 > 0.0f)
                {
                    float val1 = TEX3D_linear(g, N_g, u_ind_first, v_ind_first, L);
                    float val2 = TEX3D_linear(g, N_g, u_ind_last, v_ind_first, L);
                    float val3 = TEX3D_linear(g, N_g, u_ind_first, v_ind_last, L);
                    float val4 = TEX3D_linear(g, N_g, u_ind_last, v_ind_last, L);
                    vals[k_offset] += (val1 * uWeights_0 + val2 * uWeights_1) * vWeights_0
                                   + (val3 * uWeights_0 + val4 * uWeights_1) * vWeights_1;
         ...
         for (int j = 0; j < N_f.y; j++)
         {
            ...
            if (x*x + y*y > rFOVsq)
             ...
             else
             {
                ...
                float val4 = TEX3D(f, N_f, float(i + 2) + 0.5f, float(j) + 0.5f, float(k + 2) + 0.5f);
                g_output += (val1 * (vWeight_0 + vWeight_1) + val2 * vWeight_2) * (hWeight_0 + hWeight_1)
                          + (val3 * (vWeight_0 + vWeight_1) + val2 * vWeight_2) * hWeight_2;
...
bool project_SF(float *&g, float *f, parameters* params, bool data_on_cpu, bool volume_on_cpu, bool accum)
{
    ...
    //*/
    //fprintf(stderr, "forward project test here!!: %d, %d, %d, %d, %d, %d, %d, %d\n", N_f.x, N_f.y, N_f.z, params->volumeDimensionOrder, volume_on_cpu, N_g.x, N_g.y, N_g.z, data_on_cpu);
    ...
    freeTexture(d_data_array, d_data_txt, volume_on_cpu);
    cudaFree(dev_phis);
    //printf("freeTexture\n");
...
bool backproject_SF(float *g, float *&f, parameters* params, bool data_on_cpu, bool volume_on_cpu, bool accum)
{
    ...
    //fprintf(stderr, "back project test here!!: %d, %d, %d, %d, %d, %d, %d, %d\n", N_f.x, N_f.y, N_f.z, volume_on_cpu, N_g.x, N_g.y, N_g.z, data_on_cpu);
