...
__global__ void coneParallelWeightedHelicalBackprojectorKernel_SF(TEX_DATA g, int4 N_g, float4 T_g, float4 startVals_g, float* f, int4 N_f, float4 T_f, float4 startVals_f, const float R, const float D, const float tau, const float rFOVsq, const float* phis, const int volumeDimensionOrder, bool accum)
{
    ...
    for (int l = 0; l < N_g.x; l++)
    {
        ...
        if (centralWeight > 0.0f)
        {
            ...
            if (z_high > 1.0f)
            {
                float val1 = TEX3D_L2(g, N_g, s_ind_A, row_high + v_oneAndTwo, L);
                float val2 = TEX3D_L2(g, N_g, s_ind_A, row_high_plus_two, L);
...
__global__ void coneParallelProjectorKernel_SF(float* g, int4 N_g, float4 T_g, float4 startVals_g, TEX_DATA f, int4 N_f, float4 T_f, float4 startVals_f, float R, float D, float tau, float rFOVsq, float* phis, int volumeDimensionOrder, bool accum)
{
    ...
    if (fabs(cos_phi) > fabs(sin_phi))
    {
        ...
        for (int i = 0; i < N_f.x; i++)
        {
            ...
            if (volumeDimensionOrder == 0)
            {
                float val1 = TEX3D_L1(f, N_f, z_12, y_12, float(i) + 0.5f);
                float val2 = TEX3D_L1(f, N_f, float(k + 2) + 0.5f, y_12, float(i) + 0.5f);
                float val3 = TEX3D_L1(f, N_f, z_12, float(j_min_A + 2) + 0.5f, float(i) + 0.5f);
                float val4 = TEX3D_L1(f, N_f, float(k + 2) + 0.5f, float(j_min_A + 2) + 0.5f, float(i) + 0.5f);
            ...
            {
                float val1 = TEX3D_L1(f, N_f, float(i) + 0.5f, y_12, z_12);
                float val2 = TEX3D_L1(f, N_f, float(i) + 0.5f, y_12, float(k + 2) + 0.5f);
                float val3 = TEX3D_L1(f, N_f, float(i) + 0.5f, float(j_min_A + 2) + 0.5f, z_12);
                float val4 = TEX3D_L1(f, N_f, float(i) + 0.5f, float(j_min_A + 2) + 0.5f, float(k + 2) + 0.5f);
                g_output += (val1 * (vWeight_0 + vWeight_1) + val2 * vWeight_2) * (hWeight_0 + hWeight_1)
                         + (val3 * (vWeight_0 + vWeight_1) + val4 * vWeight_2) * hWeight_2;
    ...
    else
    {
        ...
        for (int j = 0; j < N_f.y; j++)
        {
            ...
            if (volumeDimensionOrder == 0)
            {
                float val1 = TEX3D_L1(f, N_f, z_12, float(j) + 0.5f, x_12);
                float val2 = TEX3D_L1(f, N_f, float(k + 2) + 0.5f, float(j) + 0.5f, x_12);
                float val3 = TEX3D_L1(f, N_f, z_12, float(j) + 0.5f, float(i_min_A + 2) + 0.5f);
                float val4 = TEX3D_L1(f, N_f, float(k + 2) + 0.5f, float(j) + 0.5f, float(i_min_A + 2) + 0.5f);
                g_output += (val1 * (vWeight_0 + vWeight_1) + val2 * vWeight_2) * (hWeight_0 + hWeight_1)
                         + (val3 * (vWeight_0 + vWeight_1) + val4 * vWeight_2) * hWeight_2;
            ...
            {
                float val1 = TEX3D_L1(f, N_f, x_12, float(j) + 0.5f, z_12);
                float val2 = TEX3D_L1(f, N_f, x_12, float(j) + 0.5f, float(k + 2) + 0.5f);
                float val3 = TEX3D_L1(f, N_f, float(i_min_A + 2) + 0.5f, float(j) + 0.5f, z_12);
                float val4 = TEX3D_L1(f, N_f, float(i_min_A + 2) + 0.5f, float(j) + 0.5f, float(k + 2) + 0.5f);
                g_output += (val1 * (vWeight_0 + vWeight_1) + val2 * vWeight_2) * (hWeight_0 + hWeight_1)
                         + (val3 * (vWeight_0 + vWeight_1) + val4 * vWeight_2) * hWeight_2;
...
__global__ void parallelBeamProjectorKernel_SF(float* g, int4 N_g, float4 T_g, float4 startVals_g, TEX_DATA f, int4 N_f, float4 T_f, float4 startVals_f, float rFOVsq, float* phis, int volumeDimensionOrder, bool accum)
{
    ...
    if (fabs(cos_phi) > fabs(sin_phi))
    {
        ...
        for (int i = 0; i < N_f.x; i++)
        {
            ...
            if (volumeDimensionOrder == 0)
            {
                float val1 = TEX3D_L1(f, N_f, float(m) + 0.5f, float(j_min_A) + 0.5f +  weight_1/(weight_0+weight_1), float(i) + 0.5f);
                float val2 = TEX3D_L1(f, N_f, float(m) + 0.5f, float(j_min_A + 2) + 0.5f, float(i) + 0.5f);
            ...
            {
                float val1 = TEX3D_L1(f, N_f, float(i) + 0.5f, float(j_min_A) + 0.5f + weight_1/(weight_0+weight_1), float(m) + 0.5f);
                float val2 = TEX3D_L1(f, N_f, float(i) + 0.5f, float(j_min_A + 2) + 0.5f, float(m)+0.5f);
                g_output += (weight_0 + weight_1) * val1
                         + max(0.0f, min(n_plus_half, s_ind_A + 2.0f * ds_ind_dj + C) 
                         - max(n_minus_half, s_ind_A + 2.0f * ds_ind_dj - C)) * val2;
    ...
    else
    {
        ...
        for (int j = 0; j < N_f.y; j++)
        {
            ...
            if (volumeDimensionOrder == 0)
            {
                float val1 = TEX3D_L1(f, N_f, float(m) + 0.5f, float(j) + 0.5f, float(i_min_A)+0.5f + weight_1/(weight_0 + weight_1));
                float val2 = TEX3D_L1(f, N_f, float(m) + 0.5f, float(j) + 0.5f, float(i_min_A + 2)+0.5f);
                g_output += (weight_0 + weight_1) * val1 
            ...
            {
                float val1 = TEX3D_L1(f, N_f, float(i_min_A) + 0.5f + weight_1 / (weight_0 + weight_1), float(j) + 0.5f, float(m) + 0.5f);
                float val2 = TEX3D_L1(f, N_f, float(i_min_A + 2) + 0.5f, float(j) + 0.5f, float(m) + 0.5f);
                g_output += (weight_0 + weight_1) * val1
                         + max(0.0f, min(n_plus_half, s_ind_A + 2.0f * ds_ind_di + C)
                         - max(n_minus_half, s_ind_A + 2.0f * ds_ind_di - C)) * val2;
...
__global__ void fanBeamBackprojectorKernel_SF(TEX_DATA g, int4 N_g, float4 T_g, float4 startVals_g, float* f, int4 N_f, float4 T_f, float4 startVals_f, float R, float D, float tau, float rFOVsq, float* phis, int volumeDimensionOrder, bool doWeight, bool accum)
{
    ...
    for (int l = 0; l < N_g.x; l++)
    {
        ...
        for (int k_offset = 0; k_offset < numZ; k_offset++)
        {
            float val1 = TEX3D_L2(g, N_g, ind_first, iv + float(k_offset), L);
            float val2 = TEX3D_L2(g, N_g, ind_last, iv + float(k_offset), L);
...
__global__ void fanBeamProjectorKernel_SF(float* g, int4 N_g, float4 T_g, float4 startVals_g, TEX_DATA f, int4 N_f, float4 T_f, float4 startVals_f, float R, float D, float tau, float rFOVsq, float* phis, int volumeDimensionOrder, bool accum)
{
    ...
    if (fabs(u * cos_phi - sin_phi) > fabs(u * sin_phi + cos_phi))
    {
        ...
        for (int j = 0; j < N_f.y; j++)
        {
            ...
            if (volumeDimensionOrder == 0)
            {
                float val1 = TEX3D_L1(f, N_f, float(iz) + 0.5f, float(j) + 0.5f, float(i) + 0.5f + hWeight_1/(hWeight_0 + hWeight_1));
                float val2 = TEX3D_L1(f, N_f, float(iz) + 0.5f, float(j) + 0.5f, float(i + 2)+0.5f);
            ...
            {
                float val1 = TEX3D_L1(f, N_f, float(i) + 0.5f + hWeight_1 / (hWeight_0 + hWeight_1), float(j) + 0.5f, float(iz) + 0.5f);
                float val2 = TEX3D_L1(f, N_f, float(i + 2) + 0.5f, float(j) + 0.5f, float(iz) + 0.5f);
                g_output += val1 * (hWeight_0 + hWeight_1) + val2 * hWeight_2;
    ...
    else
    {
        ...
        for (int i = 0; i < N_f.x; i++)
        {
            ...
            if (volumeDimensionOrder == 0)
            {
                float val1 = TEX3D_L1(f, N_f, float(iz) + 0.5f, float(j) + 0.5f + hWeight_1/(hWeight_0 + hWeight_1), float(i) + 0.5f);
                float val2 = TEX3D_L1(f, N_f, float(iz) + 0.5f, float(j + 2) + 0.5f, float(i) + 0.5f);
                g_output += val1 * (hWeight_0 + hWeight_1) + val2 * hWeight_2;
            ...
            {
                float val1 = TEX3D_L1(f, N_f, float(i) + 0.5f, float(j) + 0.5f+hWeight_1 / (hWeight_0 + hWeight_1), float(iz) + 0.5f);
                float val2 = TEX3D_L1(f, N_f, float(i) + 0.5f, float(j + 2) + 0.5f, float(iz)+0.5f);
                g_output += val1 * (hWeight_0 + hWeight_1) + val2 * hWeight_2;
    ...
    for (int l = 0; l < N_g.x; l++)
    {
        ...
        if (centralWeight > 0.0f)
        {
            ...
            if (z_high_A > 1.0f)
            {
                ...
                float val1 = TEX3D_L2(g, N_g, ind_first, row_high_A + v_oneAndTwo, L);
                float val2 = TEX3D_L2(g, N_g, ind_last, row_high_A + v_oneAndTwo, L);
                float val3 = TEX3D_L2(g, N_g, ind_first, row_high_plus_two_A, L);
                float val4 = TEX3D_L2(g, N_g, ind_last, row_high_plus_two_A, L);
    ...
    if (fabs(u * cos_phi - sin_phi) > fabs(u * sin_phi + cos_phi))
    {
        ...
        for (int j = 0; j < N_f.y; j++)
        {
            ...
            if (volumeDimensionOrder == 0)
            {
                float val1 = TEX3D_L1(f, N_f, z_12, float(j) + 0.5f, x_12);
                float val2 = TEX3D_L1(f, N_f, float(k + 2) + 0.5f, float(j) + 0.5f, x_12);
                float val3 = TEX3D_L1(f, N_f, z_12, float(j) + 0.5f, float(i + 2) + 0.5f);
                float val4 = TEX3D_L1(f, N_f, float(k + 2) + 0.5f, float(j) + 0.5f, float(i + 2) + 0.5f);
                g_output += (val1 * (vWeight_0 + vWeight_1) + val2 * vWeight_2) * (hWeight_0 + hWeight_1)
                         + (val3 * (vWeight_0 + vWeight_1) + val4 * vWeight_2) * hWeight_2;
            ...
            else
            {
                float val1 = TEX3D_L1(f, N_f, x_12, float(j) + 0.5f, z_12);
                float val2 = TEX3D_L1(f, N_f, x_12, float(j) + 0.5f, float(k + 2) + 0.5f);
                float val3 = TEX3D_L1(f, N_f, float(i + 2) + 0.5f, float(j) + 0.5f, z_12);
                float val4 = TEX3D_L1(f, N_f, float(i + 2) + 0.5f, float(j) + 0.5f, float(k + 2) + 0.5f);
                g_output += (val1 * (vWeight_0 + vWeight_1) + val2 * vWeight_2) * (hWeight_0 + hWeight_1)
                         + (val3 * (vWeight_0 + vWeight_1) + val4 * vWeight_2) * hWeight_2;
    ...
    else
    {
        ...
        for (int i = 0; i < N_f.x; i++)
        {
            ...
            if (volumeDimensionOrder == 0)
            {
                float val1 = TEX3D_L1(f, N_f, z_12, y_12, float(i) + 0.5f);
                float val2 = TEX3D_L1(f, N_f, float(k + 2) + 0.5f, y_12, float(i) + 0.5f);
                float val3 = TEX3D_L1(f, N_f, z_12, float(j + 2) + 0.5f, float(i) + 0.5f);
                float val4 = TEX3D_L1(f, N_f, float(k + 2) + 0.5f, float(j + 2) + 0.5f, float(i) + 0.5f);
                g_output += (val1 * (vWeight_0 + vWeight_1) + val2 * vWeight_2) * (hWeight_0 + hWeight_1)
                         + (val3 * (vWeight_0 + vWeight_1) + val4 * vWeight_2) * hWeight_2;
            ...
            else
            {
                float val1 = TEX3D_L1(f, N_f, float(i) + 0.5f, y_12, z_12);
                float val2 = TEX3D_L1(f, N_f, float(i) + 0.5f, y_12, float(k + 2) + 0.5f);
                float val3 = TEX3D_L1(f, N_f, float(i) + 0.5f, float(j + 2) + 0.5f, z_12);
                float val4 = TEX3D_L1(f, N_f, float(i) + 0.5f, float(j + 2) + 0.5f, float(k + 2) + 0.5f);
                g_output += (val1 * (vWeight_0 + vWeight_1) + val2 * vWeight_2) * (hWeight_0 + hWeight_1)
                         + (val3 * (vWeight_0 + vWeight_1) + val4 * vWeight_2) * hWeight_2;
    ...
    else
    {
        ...
        for (int l = 0; l < N_g.x; l++)
        {
            ...
            for (int k_offset = 0; k_offset < numZ; k_offset++)
            {
                ...
                if (vWeights_1 > 0.0f)
                {
                    float val1 = TEX3D_L2(g, N_g, u_ind_first, v_ind_first, L);
                    float val2 = TEX3D_L2(g, N_g, u_ind_last, v_ind_first, L);
                    float val3 = TEX3D_L2(g, N_g, u_ind_first, v_ind_last, L);
                    float val4 = TEX3D_L2(g, N_g, u_ind_last, v_ind_last, L);
