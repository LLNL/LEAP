...
void phantom::assign(phantom& other)
{
	this->clearAll();
	for (int i = 0; i < int(other.meshes.size()); i++)
	{
		/*
		float val;
		string chemForm;
		...
		this->addMesh(other.meshes[i]->triangles, other.meshes[i]->numTriangles, other.meshes[i]->val, other.meshes[i]->chemForm.c_str());
	}
	for (int i = 0; i < int(other.objects.size()); i++)
	{
		...
		float clipCone[2];
		//*/
		//bool phantom::addObject(int type, float* c, float* r, float val, float* A, float* clip, const char* chemForm)
		this->addObject(other.objects[i].type, other.objects[i].centers, other.objects[i].radii, other.objects[i].val, other.objects[i].A, other.objects[i].clip, other.objects[i].chemForm.c_str());
	}
}

void phantom::clearMeshes()
{
	for (int i = 0; i < int(meshes.size()); i++)
	{
		meshObject* object = meshes[i];
		object->reset();
		delete object;
	}
	meshes.clear();
...
bool phantom::addMesh(float* triangles, int numTriangles, float val, const char* chemForm)
{
	meshObject* object = new meshObject;
	if (object->init(triangles, numTriangles, val, chemForm) == true)
	{
		meshes.push_back(object);

		// if the first object, add its chemical formula to the list
		// otherwise only add it if it is unique
		if (materialTypes.size() == 0)
		{
			string str = object->chemForm;
			materialTypes.push_back(str);
		}
		else
		{
			string str = object->chemForm;
			bool found = false;
			for (int i = 0; i < int(materialTypes.size()); i++)
			{
				if (materialTypes[i] == str)
				{
					found = true;
					break;
				}
...
bool phantom::addObject(int type, float* c, float* r, float val, float* A, float* clip, const char* chemForm)
{
	...
	if (object.init(type, c, r, val, A, clip, chemForm) == true)
	{
...
int phantom::getMaterialType(int which)
{
	if (which >= int(objects.size()) || which < 0)
		return 0;
	for (int i = 0; i < int(materialTypes.size()); i++)
	{
		if (objects[which].chemForm == materialTypes[i])
			return i;
	}
	return 0;
}
...
bool phantom::voxelize(float* f, parameters* params_in, int oversampling)
{
	...
	else
	{
		...
		for (int n = 0; n < int(objects.size()); n++)
		{
			...
			//*/
			addObject(f, params_in, objects[n].type, objects[n].centers, objects[n].radii, objects[n].val, objects[n].A, clip_ptr, objects[n].chemForm.c_str(), oversampling);
		}
		return true;
	}
}

bool phantom::addObject(float* f, parameters* params_in, int type, float* c, float* r, float val, float* A, float* clip, const char* chemForm, int oversampling)
{
	if (c == NULL || r == NULL)
		return false;
	if (f == NULL || params_in == NULL)
		return addObject(type, c, r, val, A, clip, chemForm);
	params = params_in;
	/*
...
bool phantom::double_cone(float* f, int N_1, int N_2, int N_3, float beta, float minimum_radius)
{
	if (f == NULL || N_1 <= 1 || N_2 <= 1 || N_3 <= 1)
		return false;
	
	float minimum_radius_sq = minimum_radius * minimum_radius;

	float z_mid = 0.5 * float(N_1 - 1);
	float y_mid = 0.5 * float(N_2 - 1);
	float x_mid = 0.5 * float(N_3 - 1);

	omp_set_num_threads(num_cpu_threads());
	#pragma omp parallel for
	for (int iz = 0; iz < N_1; iz++)
	{
		int iz_shift = iz;
		if (iz >= N_1/2)
			iz_shift = iz - N_1/2;
			...
			iz_shift = iz + N_1/2;
		float z = (iz_shift - z_mid) / z_mid;
		
		float z_term = z*tan(beta*PI/180.0);
		z_term = max(minimum_radius_sq, z_term * z_term);
		
		float* zSlice = &f[uint64(iz)*uint64(N_2)*uint64(N_3)];
		for (int iy = 0; iy < N_2; iy++)
		{
			int iy_shift = iy;
			if (iy >= N_2/2)
				iy_shift = iy - N_2/2;
				...
				iy_shift = iy + N_2/2;
			float y = (iy_shift - y_mid) / y_mid;
			for (int ix = 0; ix < N_3; ix++)
			{
				int ix_shift = ix;
				if (ix >= N_3/2)
					ix_shift = ix - N_3/2;
				...
				float x = (ix_shift - x_mid) / x_mid;
				float r_sq = x*x + y*y;
				
				if (r_sq <= z_term)
					zSlice[iy*N_3 + ix] = 1.0;	
				else
					zSlice[iy*N_3 + ix] = 0.0;
			}
		}
	}
...
bool meshObject::init(float* tri, int N, float val_in, const char* chemForm_in)
{
	if (tri == NULL || N <= 0)
		return false;
	else
	{
		reset();
		triangles = new float[N*3*3];
		memcpy(triangles, tri, N*3*3*sizeof(float));
		numTriangles = N;
		val = val_in;

		if (chemForm_in != NULL && chemForm_in[0] != 0)
			chemForm = chemForm_in;
		else
			chemForm = "H2O";
		return true;
	}
...
geometricObject::geometricObject(int type_in, float* c_in, float* r_in, float val_in, float* A_in, float* clip_in, const char* chemForm_in)
{
	init(type_in, c_in, r_in, val_in, A_in, clip_in, chemForm_in);
}
...
void geometricObject::scale_saved_cone_params(float scale_x, float scale_y, float scale_z)
{
	centers_save[0] *= scale_x;
	centers_save[1] *= scale_y;
	centers_save[2] *= scale_z;

	radii_save[0] *= scale_x;
	radii_save[1] *= scale_y;
	radii_save[2] *= scale_z;
}

void geometricObject::printParameters()
{
	//ELLIPSOID = 0, PARALLELEPIPED = 1, CYLINDER_X = 2, CYLINDER_Y = 3, CYLINDER_Z = 4, CONE_X = 5, CONE_Y = 6, CONE_Z
	printf("c = (%f, %f, %f)\n", centers[0], centers[1], centers[2]);
	printf("r = (%f, %f, %f)\n", radii[0], radii[1], radii[2]);
	if (5 <= type && type <= 7)
		printf("clipCone = (%f, %f)\n", clipCone[0], clipCone[1]);
	printf("val = %f\n", val);
	printf("chemForm = %s\n", chemForm.c_str());
}

bool geometricObject::init(int type_in, float* c_in, float* r_in, float val_in, float* A_in, float* clip_in, const char* chemForm_in)
{
	reset();
...
bool phantom::scale_phantom(float scale_x, float scale_y, float scale_z)
{
	for (int i = 0; i < int(objects.size()); i++)
	{
		...
		//*
		if (objects[i].type == CONE_X)
		{
			//objects[i].radii[2] *= 1.0;
			objects[i].clipCone[0] *= scale_x;
			objects[i].clipCone[1] *= scale_x;
			objects[i].scale_saved_cone_params(scale_x, scale_y, scale_z);
		}
		else if (objects[i].type == CONE_Y)
		{
			//objects[i].radii[2] *= 1.0;
			objects[i].clipCone[0] *= scale_y;
			objects[i].clipCone[1] *= scale_y;
			objects[i].scale_saved_cone_params(scale_x, scale_y, scale_z);
		}
		else if (objects[i].type == CONE_Z)
		{
			//objects[i].radii[1] *= 1.0;
			objects[i].clipCone[0] *= scale_z;
			objects[i].clipCone[1] *= scale_z;
			objects[i].scale_saved_cone_params(scale_x, scale_y, scale_z);
		}
		else
		{
...
bool phantom::shift_phantom(float shift_x, float shift_y, float shift_z)
{
	for (int i = 0; i < int(objects.size()); i++)
	{
		objects[i].centers[0] += shift_x;
		objects[i].centers[1] += shift_y;
		objects[i].centers[2] += shift_z;
		/*
		if (objects[i].type == CONE_X)
		{
			objects[i].clipCone[0] += shift_x;
			objects[i].clipCone[1] += shift_x;
		}
		else if (objects[i].type == CONE_Y)
		{
			objects[i].clipCone[0] += shift_y;
			objects[i].clipCone[1] += shift_y;
		}
		else if (objects[i].type == CONE_Z)
		{
			objects[i].clipCone[0] += shift_z;
			objects[i].clipCone[1] += shift_z;
		}
		//*/
		...
		for (int n = 0; n < objects[i].numClippingPlanes; n++)
		{
			objects[i].clippingPlanes[n][1] += shift_x;
			objects[i].clippingPlanes[n][2] += shift_y;
			objects[i].clippingPlanes[n][3] += shift_z;
		}
		//*/
	}
	return true;
