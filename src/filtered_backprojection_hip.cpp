...
bool filteredBackprojection::execute(float* g, float* f, parameters* params, bool data_on_cpu, bool volume_on_cpu, bool accumulate)
{
	...
	else
	{
		...
		if (volume_on_cpu)
		{
			if ((cudaStatus = hipMalloc((void**)&dev_f, uint64(params->numX) * uint64(params->numY) * uint64(params->numZ) * sizeof(float))) != hipSuccess)
			{
...
bool filteredBackprojection::execute_attenuated(float* g, float* f, parameters* params, bool data_on_cpu)
{
	...
	else
	{
		...
		float* dev_f = 0;
		if ((cudaStatus = hipMalloc((void**)&dev_f, params->numX * params->numY * params->numZ * sizeof(float))) != hipSuccess)
		{
			fprintf(stderr, "hipMalloc(volume) failed!\n");
			if (dev_g != 0)
				hipFree(dev_g);
...
bool filteredBackprojection::filterProjections_Novikov(float* g, parameters* params, bool data_on_cpu)
{
	...
	float* h_1 = 0;
	float* h_2 = 0;
	hipMalloc((void**)&h_1, params->numAngles * params->numRows * params->numCols * sizeof(float));
	hipMalloc((void**)&h_2, params->numAngles * params->numRows * params->numCols * sizeof(float));
	...
	float* g_a_2 = 0;
	float* temp = 0;
	hipMalloc((void**)&g_a_1, params->numAngles * params->numRows * params->numCols * sizeof(float));
	hipMalloc((void**)&g_a_2, params->numAngles * params->numRows * params->numCols * sizeof(float));
	hipMalloc((void**)&temp, params->numAngles * params->numRows * params->numCols * sizeof(float));
