...
\item Python implementations of some iterative reconstruction algorithms: OSEM, SART, and RWLS.
\end{enumerate}
...
\item[] set\_fanBeam(numAngles, numRows, numCols, pixelHeight, pixelWidth, centerRow, centerCol, phis, sod, sdd)
\item[] set\_coneBeam(numAngles, numRows, numCols, pixelHeight, pixelWidth, centerRow, centerCol, phis, sod, sdd)
\item[] set\_modularBeam(numAngles, numRows, numCols, pixelHeight, pixelWidth, sourcePositions, detectorCenters, rowVecs, colVecs)
\end{itemize}
A description of these parameters can be found in Tables \ref{tab:stdGeometryParameters} and \ref{tab:modularParameters}.

\begin{table}[h!]
\caption{Parallel-, fan-, and cone-beam parameters.  Note that sod and sdd only apply to fan- and cone-beam geometries and the centerRow parameter has no effect on parallel- and fan-beam geometries.  Note that the phis parameter values \textit{must} be monotomic.  Internally, LEAP maintains a copy of the phis array, so if you want to make changes to it, you must either re-specify all parameters or use the \texit{set\_angles} function.  More information can be found in Section \ref{sec:CTGeometries}.} \label{tab:stdGeometryParameters}
